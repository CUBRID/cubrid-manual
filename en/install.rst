...
The installation program consists of shell scripts that contain binary; thus it can be installed automatically. The following example shows how to install CUBRID with the "CUBRID-11.0.0.0245-598c7bb-Linux.x86_64.sh" file on the Linux. 

::

    $ sh CUBRID-11.0.0.0245-598c7bb-Linux.x86_64.sh
...
As shown in the example above, after installing the downloaded file (CUBRID-11.0.0.0245-598c7bb-Linux.x86_64.sh), the CUBRID related environment variables must be set in order to use the CUBRID database. Such setting has been made automatically when logging in the concerned terminal. Therefore there is no need to re-set after the first installation. ::
...
You can install CUBRID by using rpm file that is created on CentOS 6. The way of installing and uninstalling CUBRID is the same as that of using general rpm utility. While CUBRID is being installed, a new system group (cubrid) and a user account (cubrid) are created. After installation is complete, you should log in with a cubrid user account to start a CUBRID service.::
    ...
    $ rpm -Uvh cubrid-11.0.0.0245-598c7bb-Linux.x86_64.rpm
...
**Installation Process**
    ...
    *   Decompress the compressed file to the directory to install.
        ...
        ::
            ...
            tar xvfz CUBRID-11.0.0.0245-598c7bb-Linux.x86_64.tar.gz /home1/cub_user/
