...
A method is a program written in C with built-in functions of the CUBRID database system, and is called by the **CALL** statement. 
When a method is called, the program is loaded and linked with the running application by a dynamic loader. The return value generated by the method execution is delivered to the caller.

**Limitations**
    * Currently, methods are only available in some systems of the CUBRID database, such as the :ref:`authorization-method`.
    * User-defined methods are not recommended due to many constraints and are only used in system-built methods.
