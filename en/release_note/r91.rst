...
INSERT 문의 SET 절에서 할당 표현식 안에 다른 칼럼이 있는 경우에도 평가가 가능하도록 수정(CUBRIDSUS-3112)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
 
INSERT 문의  SET 절에서 할당 표현식 안에 다른 칼럼이 있는 경우, 예를 들어 b=a+1인 경우 해당 표현식에 대한 평가가 가능하도록 수정했다.
...
위의 예에서 수정 이전 버전은 b의 값을 평가할 수 없으나, 수정 이후에는 먼저 a의 값을 0으로 할당하므로 b의 값을 1로 평가한다. 
...
CCI 정적 라이브러리를 사용하여 C 코드를 컴파일하면 링크 오류가 발생하는 문제를 수정했다.  ::
    ...
    $ gcc -o test_cci test_cci.c -m64 -I/home1/brightest/CUBRID/include -lnsl /home1/brightest/CUBRID/lib/libcascci.a -lpthread -lstdc++ 2>&1 |more

    /home/CUBRID/lib/libcascci.a(cci_util.o): In function `cci_url_match':
    /home1/build/workspace/CUBRID_9.0.0/RB-9.0.0/build_x86_64_release/cci/../../src/cci/cci_util.c:653: undefined reference to `cub_regset_malloc'
    /home1/build/workspace/CUBRID_9.0.0/RB-9.0.0/build_x86_64_release/cci/../../src/cci/cci_util.c:654: undefined reference to `cub_regset_realloc'
    ...
...
Windows 용 브로커를 통해 서버에 접속할때 cub_master가 구동되어 있지 않으면 잘못된 에러가 발생하는 문제(CUBRIDSUS-10739)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

응용프로그램이 Windows 용 브로커를 통해 서버에 접속하는 경우 cub_master 프로세스가 구동되어 있지 않으면 잘못된 에러가 발생하고 CAS가 무한 루프를 반복하는 문제를 수정했다.
...
XAConnection 인터페이스에서 HOLD_CURSORS_OVER_COMMIT 미지원(CUBRIDSUS-10800)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
