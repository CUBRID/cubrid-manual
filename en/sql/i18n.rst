...
-   There is the situation when multiple users share the same CUBRID installation and an extend is done by one of them. If that user doesn't have access
    ...
    the update wasn't made will try to do an extend on his databases, he will not be able to do this because the checksum of the library will be 
    different from then one in his databases.
