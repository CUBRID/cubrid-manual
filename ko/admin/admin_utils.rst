...
.. option:: --copy-lob-path
    ...
    이 옵션을 선택하면 대상 데이터베이스의 LOB 디렉토리의 위치를 원본 데이터베이스의 LOB 디렉토리의 위치와 동일하게 설정한다. 이 옵션을 생략하면 기본 위치는 <대상 데이터베이스 디렉토리>/lob 이다. 이 옵션은 LOB 파일을 복사하는 기능은 아니며, -B 옵션과 병행할 수 없다. ::
