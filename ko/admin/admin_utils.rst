...
.. option:: -i, --interval=SECOND
    ...
    *   **Thread_stats_counters_timers**: 클라이언트 요청을 처리하는 워커에 대한 통계 정보이다.

    *   **Thread_loaddb_stats_counters_timers**: 데이터베이스에 데이터를 로딩하는 워커에 대한 통계 정보이다 (**loaddb** 명령 사용시).
    ...
    **데몬 스레드**:
    ...
    +--------------------------+----------------+---------------------------------------------------------------------------------------+
    | ..waiter_lock_wakeup\    | Accumulator    | 데몬 스레드에 잠금이 필요한 깨우기 요청 횟수                                          |
    | \_count                  |                |                                                                                       |
    +--------------------------+----------------+---------------------------------------------------------------------------------------+
    ...
    +-------------------------------------------+---------------------------------------------------------------------------------------+
    | Deadlock_detect_daemon_thread             | 교착 상태를 찾고 시스템 정지를 막기 위해 희생자를 선택하는 백그라운드 스레드          |
    +-------------------------------------------+---------------------------------------------------------------------------------------+
