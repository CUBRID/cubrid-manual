...
.. option:: -i, --interval=SECOND
    ...
    **동시성/잠금**
    ...
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    | Time_obj_lock_acquire_time               | Complex        | 잠금 모드별로 객체를 참그는 데 소요되는 시간                          |
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    ...
    **인덱스**
    ...
    |                                          |                | | 전혀 포함하지 않은 경우의 개수                                      |
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    | Num_btree_resumes                        | Accumulator    | 지정된 인덱스 스캔 횟수를 초과하는 경우, 그 초과한 횟수               |
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    ...
    **힙**
    ...
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    | Num_heap_stats_sync_bestspace            | Accumulator    | | "Best pages" 목록의 갱신 횟수.                                      |
    |                                          |                | | "Best pages"는 여러 INSERT 및 DELETE 환경에서 여유 공간이 30% 이상인|
    |                                          |                | | 페이지를 의미한다.                                                  |
    |                                          |                | | "Best pages" 목록에는 이 페이지의 일부 정보만 저장된다.             |
    |                                          |                | | "Best pages" 목록에는 백만 페이지의 정보가 한 번에 저장된다.        |
    |                                          |                | | 이 목록은 레코드를 INSERT 할 때 검색되며 해당 페이지의 여유 공간이  |
    |                                          |                | | 없을 때 "Best pages" 목록은 갱신된다.                               |
