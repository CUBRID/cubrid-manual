...
단, 유틸리티 실행 전에 **cubrid.conf** 파일에 **communication_histogram** 파라미터를 **yes**\ 로 설정하거나, csql에서 ";se communication_histogram=yes"를 실행해야 한다. 

또한, 서버의 모든 통계 정보가 아닌 csql의 해당 연결에 대한 통계 정보를 확인하려면 먼저 CSQL의 세션 명령어 "**;.h on**\ "을 이용하여 해당 연결에 대한 통계 정보 수집을 시작한 후, **;.dump_hist** 또는 **;.x** 명령을 이용하여 통계 정보를 출력한다.
