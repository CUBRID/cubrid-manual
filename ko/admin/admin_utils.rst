...
.. option:: -i, --interval=SECOND
    ...
    **페이지 버퍼 I/O**
    ...
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    | ..wake_flush_waiter                      | Counter/timer  | BCB를 기다리는 스레드를 깨우기위한 횟수와 시간                        |
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    ...
    **이중 쓰기 버퍼 (Double write buffer)**
    ...
    | **항 목**                                | **통계 타입**  |  **설명**                                                             |
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    | ..DWB_flush_block                        | Counter/timer  | 내려쓰기 (flush)되는 블락 (block)의 갯수와 전체 쓰기 시간             |
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    ...
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    | ..DWB_flush_block_sort                   | Counter/timer  | 내려쓰기 (flush)위하여 정렬된 페이지의 갯수와 시간                    |
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    | ..DWB_decache_pages_after_write          | Counter/timer  | 내려쓰기 이후 DWB 캐시에서 제거되는 페이지들의 갯수와 시간            |
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    | ..DWB_wait_flush_block                   | Counter/timer  | | 페이지를 추가하기 위하여 블락이 내려쓰여 (flush)지기를 기다리는     |
    |                                          |                | | 횟수와 시간                                                         |
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    | ..DWB_wait_file_sync_helper              | Counter/timer  | DWB helper가 파일을 동기화하는 것을 기다리는 횟수와 시간              |
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    | ..DWB_flush_force                        | Counter/timer  | 강제로 DWB를 완전히 내려쓰기 (flush)한 횟수와 시간                    |
    +------------------------------------------+----------------+-----------------------------------------------------------------------+
    | Num_dwb_flushed_block_volumes            | Complex        | | 각각의 블락 내려쓰기 (block flush)에 의해 동기화된 파의들 갯수의    |
