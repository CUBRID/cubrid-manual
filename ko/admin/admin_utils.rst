...
.. option:: -i, --interval=SECOND
    ...
    +------------------+------------------------------------------+--------------------------------------------------------------------------------------+
    | 질의 계획        | Num_plan_cache_add                       | 캐시 entry가 새로 추가된 횟수                                                        |
    | 캐시 관련        +------------------------------------------+--------------------------------------------------------------------------------------+
    |                  | Num_plan_cache_lookup                    | 특정 키를 사용하여 lookup을 시도한 횟수                                              |
    |                  +------------------------------------------+--------------------------------------------------------------------------------------+
    |                  | Num_plan_cache_hit                       | 질의 문자열 해시 테이블에서 entry를 찾은(hit) 횟수                                   |
    |                  +------------------------------------------+--------------------------------------------------------------------------------------+
    |                  | Num_plan_cache_miss                      | 질의 문자열 해시 테이블에서 entry를 찾지 못한(miss) 횟수                             |
    |                  +------------------------------------------+--------------------------------------------------------------------------------------+
    |                  | Num_plan_cache_full                      | 캐시 entry의 개수가 허용된 최대 개수를 넘어 victim 탐색을 시도한 횟수                |
    |                  +------------------------------------------+--------------------------------------------------------------------------------------+
    |                  | Num_plan_cache_delete                    | 캐시 entry가 삭제된(victimized) 횟수                                                 |
    |                  +------------------------------------------+--------------------------------------------------------------------------------------+
    |                  | Num_plan_cache_invalid_xasl_id           | xasl_id 해시 테이블에서 entry를 찾지 못한(miss) 횟수.                                |
    |                  |                                          | 서버에서 특정 entry가 제거(victimized)되었는데, 해당 entry를 client에서              |
    |                  |                                          | 요청했을 때 발생하는 에러 횟수                                                       |
    |                  +------------------------------------------+--------------------------------------------------------------------------------------+
    |                  | Num_plan_cache_query_string_hash_entries | 질의 문자열 해시 테이블의 현재 entry 개수                                            |
    |                  +------------------------------------------+--------------------------------------------------------------------------------------+
    |                  | Num_plan_cache_xasl_id_hash_entries      | xasl id 해시 테이블의 현재 entry 개수                                                |
    |                  +------------------------------------------+--------------------------------------------------------------------------------------+
    |                  | Num_plan_cache_class_oid_hash_entries    | class oid 해시 테이블의 현재 entry 개수                                              |
    +------------------+------------------------------------------+--------------------------------------------------------------------------------------+
