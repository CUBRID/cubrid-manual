...
**dblink_auto_commit**
    ...
    **dblink_auto_commit**\ 은 분산 트랜잭션 처리의 일환으로 dblink 쿼리의 커밋을 처리하기 위한 설정이며 기본 값은 **yes**\이다. 이 파라미터 값이 **yes** 일 경우 dblink 쿼리는 실행 즉시 자동으로 커밋이 이루어지며 이 파라미터 값을 **no** 로설정하면 dblink 쿼리를 포함한 트랜잭션이 커밋을 할 때 dblink로 연결된 원격 서버의 쿼리가 커밋된다. 분산 트랜잭션은 dblink 쿼리부터 커밋되며 만일 dblink쿼리 중 하나라도 커밋에 실패하는 경우 전체 트랜잭션이 취소(rollback)된다. 다만, 분산 트랜잭션에 포함된 일부 dblink 쿼리가 커밋을 한 후 분산 트랜잭션이 커밋에 실패하더라도 이미 커밋된 dblink쿼리는 되돌릴 수 없다.
