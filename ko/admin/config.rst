...
**SSL**
    ...
    **SSL** 은 해당 브로커에 패킷 암호화 (SSL) 적용 여부를 결정하기 위한 파라미터로, **ON** 또는 **OFF** 의 값으로 설정된다. 기본값은 **OFF** 이며, 이 파라미터가 **ON** 으로 설정된 경우 브로커/CAS는 TLS를 이용한 패킷 암호화 모드로 동작한다. 이 경우 **jdbc** 등의 클라이언트들도 보안모드로 접속하여야 한다. 그렇지 않은 경우 연결 요청은 브로커에 의해서 거부된다. 반대로 이 파라미터가 **OFF** 로 설정된 경우, 보안 모드가 설정된 client로부터의 접속 요청도 거부된다.
