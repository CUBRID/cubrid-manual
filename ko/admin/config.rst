...
**use_user_hosts** 
    ...
    *   **$CUBRID/conf/hosts.conf** 파일의 형식은 **/etc/hosts** 와 동일하나 다음과 같은 몇 가지 제약이 있다.
        ...
        * 같은 ip 주소가 여러 개 있는 경우 맨 윗 줄에 있는 {ip, hostname} 정보만 사용되며, 나머지는 무시된다. ::
...
.. warning::
    ...
    cubrid_hosts.conf 파일에서 호스트 이름은 다음과 같은 형식을 따른다 (Linux 호스트명 작성 규칙)

    * 호스트 이름으로는 오직 영문자, 숫자(0 ~ 9), 하이픈('-'), 점 (".") 문자만 사용할 수 있다.
    * 호스트 이름의 첫 문자는 영문자이어야 한다.
    * 호스트 이름의 마지막 문자는 영문자와 숫자이어야 한다.
    * FQDN 형태의 호스트명이 사용 가능하다 (예: www.cubrid.com)
