...
**max_parallel_workers**
    ...
    **max_parallel_workers**\ 는 서버 전체에서 동시에 실행할 수 있는 병렬 질의 워커(parallel query worker) 스레드의 최대 개수를 설정하는 파라미터이다. 기본값은 **100**\ 이며, 최소값은 **0**, 최대값은 **1000**\ 이다.
    ...
    각 서버 스레드는 질의 실행 전에 **병렬 질의 워커 매니저(parallel query worker manager)**\ 를 통해 필요한 워커 수를 미리 예약(reserve)하며, 작업 완료되면 즉시 해제(release)한다. 만약 가용 워커가 부족하여 예약에 실패할 경우, 해당 질의는 일반적인 단일 스레드 실행 방식으로 수행된다.
...
**parallelism**
    ...
    **PARALLEL** ( *degree* ) 힌트를 사용하여 쿼리별로 병렬 처리 정도를 재정의할 수 있다. 자세한 내용은 :ref:`parallel-query`\ 를 참고한다.
