...
**max_query_per_tran**
    ...
    **max_query_per_tran** 은 트랜잭션 당 최대 쿼리 개수를 설정할때 사용되는 파라미터이다.

    기본 설정 값은 100으로 트랜잭션 당 최대 100개의 쿼리가 가능하며, 최소 1,  최대 32,767까지 설정할 수 있다.
    이 값을 너무 크게 설정하면, 트랜잭션 관련 로그 및 메모리와 스토리지가 증가하여 전체 시스템 성능에 영향을 줄 수 있으므로 주의해야 한다.
