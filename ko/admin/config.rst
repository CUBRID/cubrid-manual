...
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
|                               | vacuum_ovfp_check_duration          | 서버                    |         | int      | 744H                           |                 |
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
 ...
 **vacuum_ovfp_check_duration**\는 **vacuum_ovfp_check_threshold**\ 설정에 의해 수집된 정보를 보관할 기간을 설정한다. 설정한 기간 내에 업데이트되지 않은 정보는 삭제된다. 입력값의 단위를 생략하면 밀리초이며 기본값은 31일에 해당하는 744 시간이다. 설정할 때 단위로 ms/s/min/h를 사용할 수 있다.
...
.. note::
    ...
    *   **deduplicate_key_level**\를  **\-1**\로 지정하는 경우에는 명시적으로 *deduplicate level*\을 지정하더라도 무시되고 *deduplicate level*\은 0으로 강제된다.
