...
**max_parallel_workers**
    ...
    **max_parallel_workers**\ 는 서버 전역에서 동시에 실행 가능한 병렬 쿼리 워커(parallel query worker) 스레드의 최대 개수를 설정하는 파라미터이다. 기본값은 **100**\ 이며, 최소값은 **0**, 최대값은 **1000**\ 이다.
    ...
    이 파라미터가 **0**\ 으로 설정되면 병렬 쿼리 기능이 비활성화된다. 2 이상으로 설정하면 병렬 힙 스캔(Parallel Heap Scan), 병렬 비상관 부질의 실행(Parallel Subquery Execution), 병렬 해시 조인(Parallel Hash Join), 병렬 정렬(Parallel Sort) 등 다양한 병렬 쿼리 기능을 사용할 수 있다.

    서버는 전역 워커 풀(worker pool)을 통해 병렬 쿼리 실행 작업을 관리하며, 다수의 세션에서 동시에 병렬 쿼리를 실행하거나 단일 세션에서 다수의 병렬 연산을 수행하는 경우에도, 서버 전체에서 사용되는 병렬 워커 수는 **max_parallel_workers** 값을 초과할 수 없다.

    각 서버 스레드는 병렬 쿼리 실행 전에 전역 병렬 쿼리 워커 매니저(parallel query worker manager)로부터 필요한 워커 수를 미리 예약(reserve)하며, 작업 완료 후 이를 해제(release)한다. 예약에 실패할 경우 일반적인 단일 스레드 실행 방식으로 쿼리가 수행된다.

    병렬 쿼리 실행에 대한 자세한 내용은 :ref:`parallel-query`\ 를 참고한다.
    ...
    .. note::
        ...
        병렬 쿼리 기능을 사용하려면 충분한 CPU 코어와 메모리 리소스가 확보되어 있어야 한다. **max_parallel_workers** 값이 너무 크면 시스템 리소스 경쟁으로 인해 오히려 성능이 저하될 수 있으므로 주의한다.

    .. note::

        병렬 쿼리 실행 시 각 워커 스레드는 독립적인 메모리와 리소스를 사용하므로, 병렬 정도(parallelism degree)를 높이면 메모리 사용량이 증가한다.
...
**parallelism**
    ...
    **parallelism**\ 은 병렬 쿼리 실행 시 하나의 병렬 연산에 적용할 수 있는 병렬 처리 수준(degree of parallelism)의 상한값을 설정하는 파라미터이다. 기본값은 **4**\ 이며, 최소값은 **0**, 최대값은 **MIN(32, 시스템 코어 수)**\ 이다.
    ...
    이 파라미터가 **0**\ 으로 설정되면 쿼리에 **PARALLEL** 힌트가 명시적으로 지정되지 않는 한 병렬 실행이 비활성화된다. 2 이상으로 설정하면 병렬 힙 스캔(Parallel Heap Scan), 병렬 해시 조인(Parallel Hash Join), 병렬 정렬(Parallel Sort), 비상관 부질의(uncorrelated subquery) 등에 대해 병렬 실행이 자동으로 활성화될 수 있다.
