...
**max_parallel_workers**
    ...
    **max_parallel_workers**\ 는 서버 전역에서 동시에 실행 가능한 병렬 쿼리 워커(parallel query worker) 스레드의 최대 개수를 설정하는 파라미터이다. 기본값은 **100**\ 이며, 최소값은 **0**, 최대값은 **1000**\ 이다.
    ...
    각 서버 스레드는 병렬 쿼리 실행을 수행하기 전에 전역 병렬 쿼리 워커 매니저(parallel query worker manager)에서 필요한 워커 수를 미리 예약(reserve)하고, 작업 완료 후 해제(release)한다. 예약에 실패할 경우 일반적인 단일 스레드 실행 방식으로 쿼리가 수행된다.
...
**parallelism**
    ...
    **parallelism**\ 은 병렬 쿼리 실행 시 하나의 병렬 연산에 적용할 수 있는 병렬 처리 수준(degree of parallelism)의 상한값을 설정하는 파라미터이다. 기본값은 **4**\ 이며, 최소값은 **0**, 최대값은 **MIN(32, 시스템 코어 수)**\ 이다.
    ...
    쿼리 전체에서 사용되는 병렬 처리 수준의 합은 이 파라미터 값을 넘을 수 있으나, :ref:`max_parallel_workers <max_parallel_workers>` 값을 초과할 수는 없다.
    ...
    **PARALLEL** ( *degree* ) 힌트를 사용하여 쿼리별로 병렬 처리 정도를 재정의할 수 있다. 자세한 내용은 :ref:`parallel-query`\ 를 참고한다.
    ...
    .. note::
        ...
        **parallelism** 파라미터는 병렬 실행의 상한값을 설정하지만, 실제 병렬 실행 여부와 수준은 처리량 규칙, 테이블 크기, :ref:`max_parallel_workers <max_parallel_workers>` 설정 등 여러 요인에 따라 결정된다.
