...
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
|                               | max_connection_worker               | 서버                    |         | int      | 유효 CPU 수 / 2                |                 |
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
...
다음은 데이터베이스 서버와 관련된 파라미터로 각 파라미터의 타입과 설정 가능한 값의 범위는 다음과 같다. 연결 워커는 비차단 입출력 방식으로 여러 클라이언트 연결을 처리하고, 수신한 요청을 태스크 워커에 전달한다. 연결 워커 파라미터는 동적으로 변경할 수 없으며 서버를 재구동한 후 적용된다.
...
**max_clients**
    ...
    *   (각 브로커 노드 당 최대 50개) * (브로커 노드 2개) = 100
    ...
    브로커 이외의 일반 클라이언트가 접속한다면 그 수를 추가로 고려해야 한다. 시스템 관리자용 연결과 HA 복제 프로세스용 예약 연결은 **max_clients** 값에 더하지 않는다.
...
**max_connection_worker**
    ...
    **max_connection_worker**\ 는 클라이언트 연결의 비차단 네트워크 입출력을 처리하는 연결 워커의 최대 개수를 지정한다. 기본값은 유효 CPU 수의 절반이며, 유효 CPU 수보다 크게 설정할 수 없다. 이 값은 동시 접속 가능한 클라이언트 수를 변경하지 않는다.
...
**recv_budget_per_connection**, **send_budget_per_connection**
    ...
    **recv_budget_per_connection** 및 **send_budget_per_connection**\ 은 한 번의 입출력 처리에서 하나의 연결이 수신하거나 송신할 수 있는 최대 바이트 수를 지정한다. 기본값은 각각 **16384**\ 와 **32768**\ 이며, **0**\ 은 제한하지 않음을 의미한다. 이 제한은 한 연결이 연결 워커를 독점하지 않도록 한다.
...
.. note::
    ...
    **thread_core_count** 파라미터는 **task_group** 파라미터로 대체되었다. **thread_connection_pooling** 및 **thread_connection_timeout_seconds** 파라미터는 호환성을 위해 설정 파일에서 인식되지만 현재 연결 워커의 동작에는 영향을 주지 않는다.
...
**thread_worker_pooling**
    ...
    **thread_worker_pooling** 파라미터가 true이면, 서버 부팅 시 클라이언트 요청을 실행하는 태스크 워커가 풀링된다.
