...
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
|                               | max_connection_worker               | 서버                    |         | int      | 시스템 코어 수 / 2             |                 |
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
|                               | min_connection_worker               | 서버                    |         | int      | 4                              |                 |
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
|                               | auto_scaling_window_size            | 서버                    |         | int      | 4                              |                 |
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
|                               | recv_budget_per_connection          | 서버                    |         | int      | 16384 (16KB)                   |                 |
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
|                               | send_budget_per_connection          | 서버                    |         | int      | 32768 (32KB)                   |                 |
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
...
+-------------------------------+-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
| :ref:`thread-parameters`      | task_group                          | 서버                    |         | int      | 시스템 코어 수                 |                 |
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
|                               | task_worker                         | 서버                    |         | int      | max_clients의 값               |                 |
|                               +-------------------------------------+-------------------------+---------+----------+--------------------------------+-----------------+
...
+----------------------------+----------+--------------------+-------------+------------------+
| max_connection_worker      | int      | 시스템 코어 수 / 2 | 1           |                  |
+----------------------------+----------+--------------------+-------------+------------------+
| min_connection_worker      | int      | 4                  | 1           |                  |
+----------------------------+----------+--------------------+-------------+------------------+
| auto_scaling_window_size   | int      | 4                  | 1           | 시스템 코어 수   |
+----------------------------+----------+--------------------+-------------+------------------+
...
**min_connection_worker**
    ...
    **min_connection_worker**\ 는 데이터베이스 서버에서 연결 처리에 사용할 **connection worker**\ 의 최소 개수를 지정한다. 서버는 **max_connection_worker**\ 와 **min_connection_worker**\ 를 토대로 **connection worker**\ 의 개수를 조절한다.

**auto_scaling_window_size**

    **auto_scaling_window_size**\ 는 서버가 적절한 **connection worker**\ 의 개수를 찾을 때 탐색할 범위를 지정한다. **auto_scaling_window_size**\ 가 너무 크면 성능이 떨어질 수 있고, 너무 작으면 제대로 된 조절이 어려울 수 있다.

.. _budget_per_connection:

**recv_budget_per_connection**

    **recv_budget_per_connection**\ 은 하나의 연결이 한 번에 수신할 수 있는 bytes를 지정한다. **recv_budget_per_connection**\ 이 너무 작으면 수신이 자주 끊겨 느려질 수 있으며, 너무 크면 다른 연결들의 수신이 지연될 수 있다.

**send_budget_per_connection**

    **send_budget_per_connection**\ 은 하나의 연결이 한 번에 송신할 수 있는 bytes를 지정한다. **send_budget_per_connection**\ 이 너무 작으면 송신이 자주 끊겨 느려질 수 있으며, 너무 크면 다른 연결들의 송신이 지연될 수 있다.
...
**tcp_keepalive_idle** 
    ...
    **tcp_keepalive_idle**\ 는 TCP 네트워크 프로토콜에 TCP_KEEPIDLE 옵션값을 지정하는 파라미터로, 기본값은 **300**\ 초이다. 이 연결 간에 데이터 송수신이 일정 시간 이상 발생하지 않으면 패킷을 보내 연결을 검증한다.

**tcp_keepalive_interval** 
  
    **tcp_keepalive_interval**\ 는 TCP 네트워크 프로토콜에 TCP_KEEPINTVL 옵션값을 지정하는 파라미터로, 기본값은 **300**\ 초이다. 앞서 보낸 확인 패킷에 응답이 없을 시 패킷을 다시 보내기까지의 시간을 의미한다.

**tcp_keepalive_count** 
  
    **tcp_keepalive_count**\ 는 TCP 네트워크 프로토콜에 TCP_KEEPCNT 옵션값을 지정하는 파라미터로, 기본값은 **3**\ 이다. 확인 패킷을 보낼 횟수를 의미하며 횟수 만큼 보냈음에도 응답을 받지 못한다면 연결을 끊는다.
...
| 파라미터 이름                         | 타입   | 기본값            | 최소값  | 최대값          |
+=======================================+========+===================+=========+=================+
| task_group                            | int    | 시스템 코어 수    | 1       | 시스템 코어 수  |
+---------------------------------------+--------+-------------------+---------+-----------------+
| task_worker                           | int    | max_clients의 값  | 1       | 1048576         |
+---------------------------------------+--------+-------------------+---------+-----------------+
...
**task_group**
    ...
    **task_group**\ 파라미터에 따라 **task worker**\ 의 그룹 개수를 조정한다. 기본 값은 시스템 코어 수로 설정된다.
