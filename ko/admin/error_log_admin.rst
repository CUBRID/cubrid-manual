...
- 이 메시지는 CUBRID에서 DBLink 서버 객체를 참조(예: 연결, 수정, 삭제 등)하려고 할 때, 지정한 이름의 서버 객체가 데이터베이스에 존재하지 않을 경우 발생합니다, 즉, 서버명이 잘못 입력되었거나, 이미 삭제되었거나, 아직 생성되지 않은 경우입니다. 소유자 이름을 생략한 서버 정의문(ALTER, DROP, RENAME SERVER)은 현재 사용자가 소유한 서버만 찾으므로, 같은 이름의 서버가 다른 사용자 소유로 존재하더라도 이 경우에 해당합니다.
- 지정한 이름의 서버 객체가 존재하더라도 현재 사용자가 그 소유자도, 소유 그룹의 멤버도, DBA 도 아닌 경우에 같은 메시지가 발생합니다. 권한이 없는 서버와 존재하지 않는 서버는 구분하지 않습니다. 이 경우는 dblink 질의에서 테이블명@소유자.서버명 형태로 다른 사용자 소유의 서버를 참조할 때 나타나며, 소유자 이름을 명시한 서버 정의문은 이 코드가 아니라 "DBA, members of DBA group, and owner can perform ..." 문구로 거부됩니다.
...
- 11.4 버전부터는 권한이 없는 서버에 대한 조작이 이 코드가 아니라 :ref:`-1305 <ERROR-1305>` 로 출력됩니다. 권한이 없는 서버와 존재하지 않는 서버를 구분하지 않기 위한 변경이며, 이 코드는 하위 호환을 위해 유지됩니다.
