...
- 이 경우는 dblink 질의에서 테이블명@소유자.서버명 형태로 서버를 참조할 때와, 소유자 이름을 생략한 서버 정의문(ALTER, DROP, RENAME SERVER)에서 나타납니다. 소유자 이름을 명시한 서버 정의문은 이 코드가 아니라 "DBA, members of DBA group, and owner can perform ..." 문구로 거부됩니다.
...
- 11.4 이후 버전에서는 권한이 없는 서버에 대한 조작이 이 코드가 아니라 :ref:`-1305 <ERROR-1305>` 로 출력됩니다. 권한이 없는 서버와 존재하지 않는 서버를 구분하지 않기 위한 변경이며, 이 코드는 하위 호환을 위해 유지됩니다.
