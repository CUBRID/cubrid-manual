...
*   함수가 반환하는 에러 코드의 값이 -20002부터 -20999 사이이면, CCI API 함수에서 발생하는 에러이다.
    :c:func:`cci_get_err_msg` 함수를 이용하여 CCI 에러 코드가 나타내는 CCI 에러 메시지를 출력할 수 있다.
...
| 에러 코드명                              | 에러 메시지                                                      | 비고                                                                                                                 |
+------------------------------------------+------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------+
| CCI_ER_DBMS                              | "CUBRID DBMS Error"                                              | 서버에서 에러가 발생한 경우 함수가 반환하는 에러 코드. 실패 원인은 T_CCI_ERROR 구조체에 저장되는                     |
...
+------------------------------------------+------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------+
| CCI_ER_STRING_PARAM                      | "Invalid string argument"                                        | :c:func:`cci_prepare`, :c:func:`cci_prepare_and_execute`                                                             |
...
+------------------------------------------+------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------+
| CCI_ER_SAVEPOINT_CMD                     | "Invalid T_CCI_SAVEPOINT_CMD value"                              | :c:func:`cci_savepoint` 함수의 인자로 유효하지 않은 T_CCI_SAVEPOINT_CMD 값이 사용됨.                                 |
+------------------------------------------+------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------+
| CCI_ER_THREAD_RUNNING                    | "Thread is running"                                              | CCI_EXEC_THREAD 플래그를 주고 :c:func:`cci_execute` 를 실행하고, :c:func:`cci_get_thread_result` 를 통해 스레드의    |
|                                          |                                                                  | 실행 결과를 확인하는 경우, 아직 해당 스레드가 실행 중임을 나타냄.                                                    |
+------------------------------------------+------------------------------------------------------------------+----------------------------------------------------------------------------------------------------------------------+
