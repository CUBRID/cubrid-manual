...
*   *user-id*: 데이터베이스에 접속할 사용자 ID이다. 기본적으로 데이터베이스에는 **dba** 와 **public** 두 개의 사용자가 존재한다. 이 값이 NULL이면 url의 사용자 ID가 사용되며, 빈 문자열("")이면 **public**\이 사용자 ID로 사용된다.
*   *password*: 데이터베이스에 접속할 사용자의 암호이다. 이 값이 NULL이면 url의 암호가 사용되며, 빈 문자열("")이면 빈 문자열이 암호로 사용된다.
