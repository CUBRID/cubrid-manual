...
시스템 관리자 모드는 CSQL 인터프리터를 통해 체크 포인트를 수행하거나 트랜잭션 모니터링을 종료하고자 할 때 사용된다. 또한, 서버 접속 개수가 시스템 파라미터 max_clients의 값을 초과하더라도 CSQL 인터프리터에서 시스템 관리자 모드로 접속하면 추가로 단 하나의 연결을 허용한다.
