...
.. note::
    ...
    *  Linux 환경에서 다음과 같은 환경 변수 설정들을 통해 **CUBRID의 메모리 관리자(GLIB 메모리 관리자)**\의 힙 메모리 할당 동작을 제어할 수 있다. 자세한 내용은 https://www.gnu.org/software/libc/manual/html_node/Malloc-Tunable-Parameters 참조

        +------------------------+-----------------------------------------------------------------------------------+---------------+
        |      환경 변수         |                                          설    명                                 |     기본값    |
        +========================+===================================================================================+===============+
        | MALLOC_MMAP_MAX\_      | mmap() 시스템 호출을 사용하여 할당할 수 있는 최대 메모리 블록 수의 임계값 설정    | 65536         |
        +------------------------+-----------------------------------------------------------------------------------+---------------+
        | MALLOC_MMAP_THRESHOLD\_| mmap() 시스템 호출을 사용할지 여부를 결정하는 크기 기준 (임계값) 설정             | 128 KiB       |
        +------------------------+-----------------------------------------------------------------------------------+---------------+
        | MALLOC_TOP_PAD\_       | 힙 메모리 상단에 유지해야 할 추가 매모리 여유 공간(패딩) 크기 설정                | 0             |
        +------------------------+-----------------------------------------------------------------------------------+---------------+
        | MALLOC_TRIM_THRESHOLD\_| 해제되는 메모리를 커널에 반환할지 여부를 결정하는 임계값 설정, 임계값 초과시 반환 | 128 KiB       |
        +------------------------+-----------------------------------------------------------------------------------+---------------+
        | MALLOC_ARENA_MAX       | 프로세스에서 생성할 수 있는 최대 아레나(arena) 개수를 제한하는 설정               | core 수의 8배 |
        +------------------------+-----------------------------------------------------------------------------------+---------------+

    *  설치 패키지안의 .cubrid.sh(또는 .cubrid.csh)에는 **MALLOC_TRIM_THRESHOLD\_**\값이 0으로 설정되어 있다. 그 외의 환경 변수들은 주석 처리되어 있음.
...
.. note::
    ...
    *  **THP (Transparent Huge Pages)** 는 리눅스 커널 기능으로 Red Hat Enterprise Linux 및 그 파생 배포판(CentOS 등)을 포함한 리눅스 OS에서 기본적으로 지원되며,커널 버전 2.6.38 이후부터 도입되어 일반적으로 활성화되어 있다.이는 메모리 관리 효율을 높이기 위해 작은 메모리 페이지(4KB)를 큰 페이지(보통 2MB)로 자동 결합하여 TLB(Translation Lookaside Buffer) 오버헤드를 줄이지만, CUBRID에서는 특정 워크로드에서 성능 문제를 일으킬 수 있어 **비활성화를 권고** 하고 있다
