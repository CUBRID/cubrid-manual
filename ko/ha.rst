...
브로커는 서버에 **Read Write**, **Read Only**, **Slave Only** 이렇게 세 가지 모드 중 한 가지로 접속할 수 있으며, 사용자가 브로커 모드를 설정할 수 있다.
...
서버 연결은 **cubrid_broker.conf**\ 의 **PREFERRED_HOSTS**\ 와 **CONNECT_ORDER** 파라미터의 영향을 받으며, 다음의 순서대로 연결을 시도한다.

#.  **PREFERRED_HOSTS**\ 가 

    a.  설정된 경우, 설정된 호스트에 순차적으로 연결을 시도하고, 해당 서버의 상태가 active이면 연결 완료
    b.  설정되지 않은 경우, 연결되어 있던 서버가 있으면 해당 서버와 연결을 시도하고, 해당 서버의 상태가 active이면 연결 완료

#.  1.에서 연결 실패한 경우, **CONNECT_ORDER**\ 의 값이

    a.  **SEQ** 또는 설정되지 않은 경우, **databases.txt** 에 설정된 호스트에 순차적으로 연결을 시도하여 서버의 상태가 active이면 연결 완료
    b.   **RANDOM**\ 인 경우, **databases.txt** 에 설정된 호스트에 무작위로 연결을 시도하여 서버의 상태가 active이면 연결 완료

#.  2.에서 연결 실패한 경우, **CONNECT_ORDER**\ 의 값이 

    a.  **SEQ** 또는 설정되지 않은 경우, **databases.txt** 에 설정된 호스트에 순차적으로 연결을 시도하여 최초 연결 가능한 서버와 연결 완료
    b.  **RANDOM**\ 인 경우, **databases.txt** 에 설정된 호스트에 무작위로 연결을 시도하여 최초 연결 가능한 서버와 연결 완료 

다음 그림은 **PREFERRED_HOSTS**\ 와 **CONNECT_ORDER**\ 가 설정되지 않았을 때 db-host 설정을 통해 호스트에 연결하는 예이다.
...
액티브 서버와 연결된 후에는 스탠바이 서버가 있어도 연결은 끊기지 않으며, **cubrid broker reset** 명령을 실행해야만 기존 연결을 끊고 새롭게 스탠바이 서버에 연결할 수 있다. Read Only 브로커에 쓰기 요청이 전달되면 브로커에서 오류가 발생하므로, 액티브 서버와 연결되어도 읽기 서비스만 가능하다.

서버 연결은 **cubrid_broker.conf**\ 의 **PREFERRED_HOSTS**\ 와 **CONNECT_ORDER** 파라미터의 영향을 받으며, 다음의 순서대로 연결을 시도한다.

#.  **PREFERRED_HOSTS**\ 가 

    a.  설정된 경우, 설정된 호스트에 순차적으로 연결을 시도하고, 해당 서버의 상태가 standby이면 연결 완료
    b.  설정되지 않은 경우, 연결되어 있던 서버가 있으면 해당 서버와 연결을 시도하고, 해당 서버의 상태가 standby이면 연결 완료

#.  1.에서 연결 실패한 경우, **CONNECT_ORDER**\ 의 값이

    a.  **SEQ** 또는 설정되지 않은 경우, **databases.txt** 에 설정된 호스트에 순차적으로 연결을 시도하여 서버의 상태가 standby이면 연결 완료
    b.  **RANDOM**\ 인 경우, **databases.txt** 에 설정된 호스트에 무작위로 연결을 시도하여 서버의 상태가 standby이면 연결 완료
...
#.  1.에서 연결 실패한 경우, **CONNECT_ORDER**\ 의 값이
    ...
    a.  **SEQ** 또는 설정되지 않은 경우, **databases.txt** 에 설정된 호스트에 순차적으로 연결을 시도하여 서버의 상태가 standby이면 연결 완료
    b.  **RANDOM**\ 인 경우, **databases.txt** 에 설정된 호스트에 랜덤으로 연결을 시도하여 서버의 상태가 standby이면 연결 완료

#.  standby 상태의 서버가 존재하지 않으면 연결 실패
...
CUBRID HA 그룹 내에서 사용할 그룹 이름과 failover의 대상이 되지 않는 멤버 노드들의 호스트 이름을 명시한다. @ 구분자로 나누어 @ 앞이 그룹 이름, @ 뒤가 멤버 노드들의 호스트 이름이다. 여러 개의 호스트 이름은 쉼표(,) 또는 콜론(:)으로 구분한다. 기본값은 **NULL** 이다.
...
**SYNC**, **SEMISYNC**, **ASYNC**\ 를 값으로 설정할 수 있다. **ha_node_list**\ 에 지정한 노드의 수만큼 설정해야 하고 순서가 같아야 한다. 쉼표(,) 또는 콜론(:)으로 구분한다. 레플리카 노드는 이 값의 설정과 관계없이 항상 ASNYC 모드로 동작한다.

자세한 내용은 :ref:`log-multiplexing` 를 참고한다.

**ha_copy_log_base**

복제 로그를 저장할 위치를 지정한다. 기본값은 **$CUBRID_DATABASES**\ 이다.
...
복제 로그의 최대 보존 개수를 지정한다. 기본값은 1이다. 슬레이브 노드 또는 레플리카 노드를 원본으로 하여 :ref:`rebuilding-replication`\ 이 필요한 경우 복제 재구축이 진행되는 동안 추가되는 복제 로그가 **ha_copy_log_max_archives** 의 지정에 의해 삭제되어선 안 되므로, **ha_copy_log_max_archives**\ 의 값을 적당히 큰 값으로 지정해야 한다. 이 경우를 제외하고는 디스크의 공간 낭비를 방지하기 위해 이 값을 기본값인 1로 유지할 것을 권장한다.
...
마스터와 레플리카 사이의 데이터 복제 반영 시간 간격을 지정한다. ms, s, min, h 단위를 지정할 수 있으며 각각 milliseconds, seconds, minutes, hours를 의미한다. 단위 생략 시 기본 단위는 밀리초(ms)이다. 기본값은 0이다.
...
**RW** (Read Write), **RO** (Read Only), **SO** (Slave Only)를 값으로 설정할 수 있다. 자세한 내용은 :ref:`broker-mode`\ 를 참고한다.
...
| 대표적인 운영 작업  | 시나리오                                  | 고려 사항                      |
+=====================+===========================================+================================+
| 레플리카 노드에서   | 레플리카 노드에 복제되는 시간 간격을      | cubrid_ha.conf의               |
| 복제 지연 설정      | 지정하고, 특정 시간에 복제가 정지되게     | ha_replica_delay와             |
|                     | 한다.                                     | ha_replica_time_bound를        |
|                     |                                           | 지정한다.                      |
+---------------------+-------------------------------------------+--------------------------------+
...
|       |                                                  |              |                                                     | 참고한다.                                                            |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 78    | 내부 에러: an I/O error occurred                 | fatal        | 복제 로그 읽기 실패                                 | cubrid applyinfo 유틸리티를 통해 복제 로그를 확인한다.               |
|       | while reading logical log page                   |              |                                                     |                                                                      |
|       | ? (physical page ?) of ?                         |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 81    | 내부 에러: logical log page ? may be corrupted.  | fatal        | 복제 로그 복사 프로세스가 연결된 데이터베이스       | 복제 로그 복사 프로세스가 연결된 데이터베이스 서버 프로세스의        |
|       |                                                  |              | 서버 프로세스로부터 복사한 복제 로그 페이지의 오류  | 오류 로그를 확인한다. 이 오류 로그는 $CUBRID/log/server에 위치한다.  |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
...
| 코드  |                                                  |              |                                                     |                                                                      |
+=======+==================================================+==============+=====================================================+======================================================================+
| 89    | 로그 ?는 주어진 데이터베이스에                   | error        | 기존에 복제되었던 로그와 현재 복제하려는            | 복제 로그 복사 프로세스가 연결한 데이터베이스 서버/호스트 정보를     |
|       | 속하지 않습니다.                                 |              | 로그가 다름                                         | 확인한다.  연결하려는 데이터베이스 서버/호스트 정보를 변경해야 하는  |
...
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 79    | 내부 에러: an I/O error occurred writing         | fatal        | 복제 로그 쓰기 실패                                 | 내부적으로 복구된다.                                                 |
|       | logical log page ?(physical page ?) of ?.        |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 80    | ?의 logical log page ? (physical page ?) 쓰는    | fatal        | 파일 시스템 공간 부족으로                           | 디스크 파티션 내 여유                                                |
|       | 도중 시스템 디바이스의 공간이 부족합니다.        |              | 복제 로그 쓰기 실패                                 | 공간이 있는지 확인한다.                                              |
|       | ? 바이트 이상은 쓸 수 없습니다.                  |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
...
| 코드  |                                                  |              |                                                     |                                                                      |
+=======+==================================================+==============+=====================================================+======================================================================+
| 78    | 내부 에러: an I/O error occurred while reading   | fatal        | 아카이브 중 복제 로그 읽기 실패                     | cubrid applyinfo 유틸리티를 통해 복제 로그를 확인한다.               |
|       | logical log page ? (physical page ?) of ?.       |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 79    | 내부 에러: an I/O error occurred while writing   | fatal        | 아카이브 로그 쓰기 실패                             | 내부적으로 복구된다.                                                 |
|       | logical log page ? (physical page ?) of ?.       |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 81    | 내부 에러: logical log page ? may be corrupted.  | fatal        | 아카이브 중 복제  로그 오류 발견                    | cubrid applyinfo 유틸리티를 통해 복제 로그를                         |
|       |                                                  |              |                                                     | 확인한다.                                                            |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 98    | ?에서 ?까지의 페이지들을 archive하기 위한        | fatal        | 아카이브 로그 파일 생성 실패                        | 디스크 파티션 내 여유 공간이 있는지 확인한다.                        |
|       | archive 로그 ?를 생성할 수 없습니다.             |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 974   | ?에서 ?까지의 페이지들을 archive하기 위한        | notification | 아카이브 로그 파일 정보                             | 이 오류 메시지는 생성된 아카이브 로그 정보를 위해                    |
|       | archive 로그 ?를 생성했습니다.                   |              |                                                     | 기록되는 것이므로 조치 사항은 없다.                                  |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
...
| 코드  |                                                  |              |                                                     |                                                                      |
+=======+==================================================+==============+=====================================================+======================================================================+
| 1037  | log writer: log writer가 시그널에 의해           | error        | 지정된 시그널에 의해 copylogdb 프로세스 종료        | 내부적으로 복구된다.                                                 |
|       | 종료됩니다.                                      |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
...
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 1038  | log applier: log applier가 시작되었습니다.       | error        | applylogdb 초기화 성공 후 정상 시작                 | 이 오류 메시지는 복제 로그 반영 프로세스의 시작 정보를               |
|       | required LSA: ?|?. last committed LSA: ?|?.      |              |                                                     | 나타내기 위해 기록되는 것이므로 조치 사항은 없다.                    |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
...
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 17    | 내부 에러: 이미 해제된 볼륨 ?의 ? 페이지에 대한  | fatal        | 복제 로그에 포함되지 않은 로그 페이지를             | cubrid applyinfo 유틸리티를 통해 복제 로그를 확인한다.               |
|       | 읽기 시도.                                       |              | 읽기 시도                                           |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 81    | 내부 에러: logical log page ? may be corrupted.  | fatal        | 기존 복제 반영 중이던 로그와 현재 로그가            | cubrid applyinfo 유틸리티를 통해 복제 로그를 확인한다.               |
|       |                                                  |              | 불일치 또는 복제 로그 레코드 오류                   |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 82    | 로그 디스크 볼륨/파일 ?을(를) 마운트할 수        | error        | 복제 로그 파일이 존재하지 않음                      | 복제 로그 존재 유무를 확인한다.                                      |
|       | 없습니다.                                        |              |                                                     | cubrid applyinfo 유틸리티를 통해 복제 로그를 확인한다.               |
|       |                                                  |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 97    | 내부 에러: unable to find log page ? in          | error        | 로그 페이지가 복제 로그에 존재하지 않음             | cubrid applyinfo 유틸리티를 통해 복제 로그를 확인한다.               |
|       | log archives.                                    |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 897   | 압축 해제 오류입니다.                            | error        | 로그 레코드 압축 해제 실패                          | cubrid applyinfo 유틸리티를 통해 복제 로그를 확인한다.               |
|       |                                                  |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 1028  | log applier: Archive 로그에 예상치 못한          | error        | 아카이브 로그에 잘못된 로그 레코드가 포함           | cubrid applyinfo 유틸리티를 통해 복제 로그를 확인한다.               |
|       | EOF 로그 레코드가 있습니다. LSA: ?|?.            |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 1029  | log applier: 잘못된 로그 페이지/오프셋. page     | error        | 잘못된 로그 레코드가 포함                           | cubrid applyinfo 유틸리티를 통해 복제 로그를 확인한다.               |
|       | HDR: ?|?, final: ?|?, append LSA: ?|?, EOF LSA:  |              |                                                     |                                                                      |
|       | ?|?, ha file status: ?, is end-of-log: ?.        |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 1030  | log applier: 잘못된 로그 레코드. LSA: ?|?, forw  | error        | 로그 레코드 헤더 오류                               | cubrid applyinfo 유틸리티를 통해 복제 로그를 확인한다.               |
|       | LSA: ?|?, backw LSA: ?|?, Trid: ?, prev tran     |              |                                                     |                                                                      |
|       | LSA: ?|?, type: ?.                               |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
...
| 코드  |                                                  |              |                                                     |                                                                      |
+=======+==================================================+==============+=====================================================+======================================================================+
| 72    | 트랜잭션이(인덱스 ?, ?@?|?) 시스템에 의해        | error        | 데드락 등에 의해 복제 반영 실패                     | 내부적으로 복구된다.                                                 |
|       | 취소되었습니다.                                  |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 111   | 당신의 트랜잭션은 서버 failure 혹은 모드         | error        | 복제를 반영하려는 데이터베이스 서버 프로세스 종료   | 내부적으로 복구된다.                                                 |
|       | 변경으로 인해 취소되었습니다.                    |              | 또는 모드 변경에 의해 복제 반영 실패                |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
...
|       |                                                  |              | 연결 종료                                           |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 1027  | log applier: ?에서 ?로 복제 반영 상태를 변경하지 | error        | 복제 반영 상태 변경 실패                            | 내부적으로 복구된다.                                                 |
|       | 못하였습니다.                                    |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 1031  | log applier: Schema 복제 로그 반영에             | error        | SCHEMA 복제 반영 실패                               | 복제 불일치 여부를 확인하고 불일치 시                                |
|       | 실패하였습니다. class: ?, schema: ?,             |              |                                                     | HA 복제 재구성을 실행한다.                                           |
|       | internal error: ?.                               |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 1032  | log applier: Insert 복제 로그 반영에             | error        | INSERT 복제 반영 실패                               | 복제 불일치 여부를 확인하고 불일치 시                                |
|       | 실패하였습니다. class: ?, schema: ?,             |              |                                                     | HA 복제 재구성을 실행한다.                                           |
|       |                                                  |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 1033  | log applier: Update 복제 로그 반영에             | error        | UPDATE 복제 반영 실패                               | 복제 불일치 여부를 확인하고 불일치 시                                |
|       | 실패하였습니다. class: ?, schema: ?,             |              |                                                     | HA 복제 재구성을 실행한다.                                           |
|       | internal error: ?.                               |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
| 1034  | log applier: Delete 복제 로그 반영에             | error        | DELETE 복제 반영 실패                               | 복제 불일치 여부를 확인하고 불일치 시                                |
|       | 실패하였습니다. class: ?, schema: ?,             |              |                                                     | HA 복제 재구성을 실행한다.                                           |
|       | internal error: ?.                               |              |                                                     |                                                                      |
+-------+--------------------------------------------------+--------------+-----------------------------------------------------+----------------------------------------------------------------------+
...
*   **target_host** : 복제 재구축을 위한 원본 노드(주로 마스터 노드)의 호스트명으로, **/etc/hosts** 에 등록되어 있어야 한다. 슬레이브 노드는 마스터 노드 또는 레플리카 노드를 원본으로 하여 복제 재구축이 가능하며, 레플리카 노드는 슬레이브 노드 또는 또 다른 레플리카 노드를 원본으로 하여 복제 재구축이 가능하다. 슬레이브 노드 또는 레플리카 노드를 원본으로 하여 복제 재구축을 수행하고자 하는 경우 cubrid_ha.conf의 :ref:`ha_copy_log_max_archives <ha_copy_log_max_archives>` 파라미터 값을 적당히 크게 설정해야 한다.
