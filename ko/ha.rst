...
데이터베이스 서버에 동시에 연결할 수 있는 일반 클라이언트의 최대 수를 지정하는 파라미터이다. 기본값은 **100**\ 이다. 시스템 관리자용 연결 1개와 HA 토폴로지에 따라 필요한 복제 로그 복사 및 반영 프로세스의 연결은 별도로 예약된다.

따라서 HA 복제 프로세스의 수를 **max_clients** 값에 더할 필요는 없다. 다만 failover가 일어날 때 다른 노드에 접속하고 있던 일반 클라이언트가 해당 노드에 집중될 수 있으므로 이를 고려하여 설정해야 한다.
