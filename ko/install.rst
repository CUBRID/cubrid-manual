...
*   WSL(Windows Subsystem for Linux) 설치 및 활성화
    ...
    CUBRID 설치 프로그램은 WSL 설치를 자동으로 수행하지 않으므로, WSL 활성화 상태를 체크하고 활성화 하는 기능만 제공한다.
...
**1단계: 설치 옵션 선택**
    ...
    *   바탕화면 바로가기 생성 여부
    *   샘플 데이터베이스 생성 여부
    *   WSL2 모드 선택 (체크시 WSL2 모드로 설치, 체크 해제시 WSL1 모드로 설치) 
...
**3단계: 설치 완료**
    ...
    *   설치 프로그램은 설치와 제거 기능만 제공된다. 제거 시에는 WSL 배포판과 Tray 애플리케이션을 제거되어,
        배포판 내부의 데이터베이스 및 설정 파일은 모두 삭제되므로, 제거 전에 데이터베이스를 백업해야 한다.
    *   업그레이드 및 기타 사용 방법은 Linux 와 동일하게 진행하면 된다.
...
.. note:: 
    ...
    마찬가지로 지동 실행 목록에 Tray 애플리케이션을 등록한 경우 중단하려면 "설정 > 앱 > 시작 프로그램 > **cubrid_tray_app.exe**" 를 끔으로 설정 한다.

**배포판 관련**

    *   설치 프로그램은 배포판 이름을 설정할 수 있으며, 배포판 이름은 Windows 명령 프롬프트에서 wsl -l 명령으로 확인 가능하다.
    ...
    *   배포판 안으로 접속하려면 Windows 명령 프롬프트에서 wsl -d <배포판 이름> -u cubrid명령으로 접속 가능하다.
