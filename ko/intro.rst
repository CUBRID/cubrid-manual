...
TDE (Transparent Data Encryption) 키 파일은 엔진 레벨에서의 데티어베이스 암호화를 위한 키들을 담고 있다. 파일 내의 키들은 TDE 유틸리티를 사용하여 관리된다.  이에 대한 자세한 설명은 :ref:`tde-file-based-key` 와 :ref:`TDE 유틸리티<tde-utility>` 를 참고한다.
