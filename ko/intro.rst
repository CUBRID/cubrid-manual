...
사용자는 데이터베이스에 추가할 볼륨 타입을 데이터(data), 임시(temp), 인덱스(index) 중 하나의 용도로 지정하여 효율적으로 관리할 수 있다. 별도로 데이터 용도를 지정하지 않는 경우에는 범용(generic) 볼륨으로 지정되며, 범용 볼륨은 데이터와 인덱스를 저장한다. 볼륨 파일이 자동으로 증가하는 경우에도 범용 볼륨으로 지정된다.
