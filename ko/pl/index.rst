...
저장 함수/프로시저는 높은 생산성과 사용성을 갖는다. 한번 만들어 놓으면 SQL에서 호출하여 계속 사용할 수 있기 때문이다.
...
PL/CSQL 프로그래밍 언어는 CUBRID SQL의 절차적 언어로의 확장이다 (Procedural Language extension of CUBRID SQL).
선언적 언어인 SQL만으로는 구현하기 힘든 조건문, 반복문, 변수, 에러 처리, 내부 프로시저/함수를 통한 모듈화 같은
절차적 언어의 특성을 PL/CSQL이 지원한다.
...
Java 저장 함수/프로시저는 뛰어난 상호 운용성과 이식성을 갖는다. Java 저장 함수/프로시저는 Java 가상 머신을 사용하므로, 시스템에 Java 가상 머신을 사용할 수만 있다면 언제 어디서나 사용할 수 있기 때문이다.
