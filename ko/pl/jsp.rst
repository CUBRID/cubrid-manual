...
Call Specification을 사용하면 SQL 문이나 응용 프로그램에서 Java 함수 이름, 인자 타입, 반환 값 및 반환 값의 타입에 접근할 수 있다.
...
자바 저장/프로시저를 구현할 때 자바 애플리케이션을 개발하는 것과 동일하게 다음의 JDBC 인터페이스를 이용하여 질의문을 수행할 수 있다.
...
.. note::
    ...
    *    **java.sql.ResultSet** 은 함수의 입력 인자로 사용할 수 없으며, 이를 IN 인자로 전달할 경우에는 오류가 발생한다.
    *    Java가 아닌 환경에서 **ResultSet** 을 반환하는 함수를 호출할 경우에도 오류가 발생한다.
...
* 반환 값 및 IN/OUT 파라미터의 타입 자릿수는 Java에서 무시하고 타입만 맞추어 그대로 데이터베이스에 전달한다.
* 저장 프로시저는 다른 저장 프로시저를 호출하거나 재귀적으로 자신을 호출할 수 있다. 최대 중첩 깊이는 16이다.

반환 값 및 IN/OUT 파라미터에 대한 타입 자릿수 제한사항
--------------------------------------------------------

반환 값과 IN/OUT의 데이터 타입에 자릿수를 한정하는 경우, 
...
Call Specification에서는 SQL의 데이터 타입과 Java의 파라미터, 반환 값의 데이터 타입이 맞게 대응되어야 한다.
