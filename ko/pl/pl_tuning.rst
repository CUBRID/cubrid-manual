...
쿼리에서 실행하는 저장 함수의 불필요한 반복 호출은 성능을 저하시킬 수 있다. 따라서 저장 함수 호출을 최적화하기 위해 다음과 같은 방법을 고려해야 한다.
...
CUBRID에서 기본적으로 제공하는 내장 함수는 (:doc:`/sql/function`) CUBRID의 쿼리 실행 동작에 맞춰 최적화되어 효율적으로 동작한다.
