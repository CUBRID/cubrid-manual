...
하나의 이름 아래에 모아서 선언, 정의할 수 있도록 하는 기능이다. 패키지를 사용하여 얻을 수 있는 효과는 아래와 같다.

* 관련 있는 선언과 정의를 하나의 패키지에 모아 모듈화함으로써 유지 보수에 도움을 준다.
* 모듈의 명세(선언)와 구현(정의)을 분리하고 모듈의 사용은 명세를 통해서만 하도록 함으로써 세부 구현에 대한 의존성을 줄인다.
* 패키지에 선언된 변수, 상수, 커서는 한 세션 동안 유지되는 상태를 제공한다.

패키지가 하나 이상의 변수, 상수, 커서를 갖는 경우에 그 패키지는 상태를 갖는 패키지라고 부른다.
상태를 갖는 패키지에 대해서는 각 세션에서 그 패키지를 최초로 참조할 때 상태 인스턴스를 한번 생성하고 초기화한다.
세션이 종료될 때 패키지 상태 인스턴스에 포함된 커서는 닫히고 상태 인스턴스에 사용된 시스템 자원은 회수된다.
패키지의 상태가 서로 다른 세션 간에 공유되는 일은 없다.
...
CREATE PACKAGE 문을 사용해서 이름과 타입 정보로 구성된 명세를 선언하고,
CREATE PACKAGE BODY 문을 사용해서 명세에 선언된 커서, 프로시저, 함수를 구현한다.
DROP PACKAGE 문을 사용해서 패키지 전체나 구현을 삭제할 수 있고,
ALTER PACKAGE 문을 사용해서 패키지 명세나 구현 혹은 전체를 다시 컴파일 할 수 있다.
...
CREATE PACKAGE 문은 패키지 사용자가 참조할 수 있는 항목들을 선언한다.
이름과 타입 정보들을 제공하는 것이 주된 기능이지만 변수의 초기값이나 커서의 실행문처럼 구현에 관련된 정보도 일부 포함할 수 있다.
...
변수 선언에는 선택적으로 초기값을 지정할 수 있다. 반면, 상수 선언에는 초기값 지정이 필수이다.
이 초기값은 패키지가 세션별로 상태 인스턴스를 생성할 때 대입된다.
초기값이 지정되지 않은 변수는 NULL로 초기화된다.
변수와 상수 모두 NOT NULL 수식어를 지정할 수 있다. NOT NULL이 지정된 변수나 상수에 NULL 값을 대입하면 실행시간 에러가 발생한다.

커서 선언에는 선택적으로 실행할 SELECT 문을 지정할 수 있다.
지정하지 않았을 때는 RETURN 절을 포함해서 커서가 반환하는 레코드의 타입을 명시해야 한다.
SELECT 문을 지정했을 때는 RETURN 절을 포함할 수도 있고 하지 않을 수도 있다.
커서에 IN 인자를 지정할 수 있다. 그러나, OUT 또는 INOUT 인자는 지원하지 않는다.

프로시저나 함수 선언은 이름과 타입 정보(인자 타입, 리턴 타입)를 제공하지만 구현(body)을 제공할 수는 없다.
구현 body는 CREATE PACKAGE BODY 문을 이용해서 제공한다.
함수의 경우 DETERMINISTIC 혹은 NOT DETERMINISTIC 수식어를 사용하여 :ref:`결정적 함수 <pl-deterministic>`\인지 여부를 지정할 수 있다.
둘 중 아무 것도 지정하지 않았을 때는 NOT DETERMINISTIC을 지정한 것처럼 동작한다.

레코드 타입은 주로 커서의 리턴 타입을 지정하기 위해 필요하다.
즉, 커서가 리턴하는 레코드가 어떤 이름과 타입의 컬럼들로 이루어져 있는지 지정한다.
레코드 타입은 :ref:`%ROWTYPE <percent_rowtype>`\을 통해서 사용할 수 있지만 위의 *record_type_def* 문법대로 정의해서 사용할 수도 있다.
레코드 컬럼을 나타내는 필드는 각각 NOT NULL 수식어와 디폴트값을 가질 수 있다.

커서 리턴 타입 뿐만 아니라 PL/CSQL 문에서 타입 지정이 필요한 모든 위치에 레코드 타입을 쓸 수 있다.
...
* CREATE PACKAGE 문에서 선언한 프로시저와 함수의 구현(body)을 제공
* CREATE PACKAGE 문에서 커서 선언에 실행문을 포함하지 않았을 때 CREATE PACKAGE BODY 문에서 제공
* 패키지 상태 인스턴스 초기화 코드를 제공 (위 문법에서 *pb_initialize_section*).
* 패키지 구현 내부적으로만 사용하고 패키지 사용자가 사용할 수 없는 패키지 private 항목들 정의
...
PL/CSQL 문에서는 패키지에서 선언한 모든 항목을 사용할 수 있는 반면, 일반 SQL 문에서는 프로시저와 함수만 사용 가능하며
그 중에서도 인자 타입과 리턴 타입을 SQL에서 사용할 수 있는 경우로만 사용이 제약된다.
예를 들어, 패키지 안에서 선언된 함수가 인자 타입이나 리턴 타입으로 레코드 타입을 갖는 경우에는 일반 SQL 문에서 그 함수를 호출할 수 없다.
