...
본 문서는 CUBRID 9.0(빌드번호 9.x.x.xxxx)에 관한 유용한 정보를 포함한다.
릴리스 노트의 최신 버전은 `CUBRID 릴리스 노트 사이트 <http://release.cubrid.org/ko>`_ 에서 확인할 수 있다.
...
특정 행 집합에 대해 다양한 통계를 얻을 수 있도록 OVER라는 새로운 분석 절을 함께 사용하는 분석 함수들을
추가했다.
...
9.0 Beta 이전 버전과 DB 볼륨이 호환되지 않으므로 cubrid unloaddb/loaddb를 이용하여 데이터를 마이그레이션해야 한다.
보다 자세한 사항은 매뉴얼 및 본 릴리스 노트의 "CUBRID 9.0 Beta로 업그레이드하는 방법" 절을 참고한다.
...
+-----------------------------+----------------------------+-------------+----------+ 
| data_buffer_pages           | data_buffer_size           | 100M(페이지 | 512M     | 
|                             |                            | 크기=4K)    |          | 
+-----------------------------+----------------------------+-------------+----------+ 
| log_buffer_pages            | log_buffer_size            | 200K(페이지 | 4M       | 
|                             |                            | 크기=4K)    |          | 
|                             |                            |             |          |
+-----------------------------+----------------------------+-------------+----------+ 
| sort_buffer_pages           | sort_buffer_size           | 64K(페이지  | 2M       | 
|                             |                            | 크기=4K)    |          | 
+-----------------------------+----------------------------+-------------+----------+ 
| index_scan_oid_buffer_pages | index_scan_oid_buffer_size | 16K(페이지  | 64K      | 
|                             |                            | 크기=4K)    |          | 
+-----------------------------+----------------------------+-------------+----------+ 
