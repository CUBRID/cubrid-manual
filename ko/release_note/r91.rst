...
모니터링
--------
	* 복제 로그 반영 상태를 출력할 때 복제 지연 상태를 출력하도록 개선했다.
	* 각 트랜잭션마다 질의의 수행 상태를 출력하는 기능을 추가했다.
	* 특정 SQL을 수행하는 트랜잭션을 지정하여 트랜잭션을 제거하는 기능을 추가했다.
	* 질의 수행이 지정한 시간을 초과하면 로그에 질의 실행 정보를 기록하게 하는 기능을 추가했다.
