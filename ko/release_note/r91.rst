...
[JDBC][CCI] 자동 커밋 모드에서 배열 처리 함수가 각 질의문을 수행할 때마다 커밋하도록 수정(CUBRIDSUS-6148)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
[CCI] 배열/배치 처리 함수에 대한 에러 처리 방식 수정(CUBRIDSUS-9364)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
TRUNCATE 테이블 또는 인덱스 재생성 이후 테이블 접근 성능 저하 개선(CUBRIDSUS-6501)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

큰 크기의 테이블을 TRUNCATE하거나 큰 인덱스를 재생성한 이후에 해당 테이블에 대한 접근 성능이 저하되는 문제를 개선했다.
...
DEFAULT와 NOT NULL 제약 조건을 가진 ENUM 타입 칼럼에 인덱스가 존재하는 경우 해당 칼럼에 대한 ORDER BY 정렬이 정상 수행되지 않는 현상(CUBRIDSUS-10841)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
[CCI] pool_prepared_statement 속성이 off인 상태로  DATASOURCE를 사용할 때 cci_datasource_release() 이후 cci_execute()를 호출하면 응용 프로그램이 비정상 종료되는 현상(CUBRIDSUS-10847)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

pool_prepared_statement 속성이 off인 상태로 DATASOURCE를 사용할 때 cci_datasource_release() 이후 닫힌 핸들로 cci_execute()를 호출하면 비정상 종료되었으나, 오류가 발생하도록 수정했다.
    
[CCI] cci_prepare 함수 수행 도중 내부적으로 PREPARE를 재시도하는 경우에 발생하는 메모리 누수 현상 수정(CUBRIDSUS-10053)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

[CCI] prepared statement를 실행하는 도중 COMMUNICATION 오류가 발생하는 현상(CUBRIDSUS-10840)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

자동 커밋 모드가 ON인 상태로 cci_execute()를 실행하는 도중 내부 루틴에서 CAS_ER_STMT_POOLING 오류가 발생하면 드라이버가 prepare를 내부적으로 재시도하는것과 동시에 CAS가 재시작되어 통신 오류(-20004, CCI_ER_COMMUNICATION)가 발생하는 현상을 수정했다. 
