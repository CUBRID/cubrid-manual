...
CUBRID 11.4은 새로운 기능, 중요한 변경 사항 및 개선 사항이 포함된 최신 안정화 버전이다.
...
사용자를 그룹에 구성원을 추가하는 SQL 구문 추가
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

#. 11.4 이전 버전에서 사용자를 그룹에 추가하기 위해서는 DROP USER 후 CREATE USER GROUPS를 실행하는 것이 필요함.
...
Opitimizer가 더 정확한 실행 계획을 수립할 수 있도록 통계 수집시 각 컬럼의 고유 값 개수 수집하는 기능을 추가함. 수집된 각 컬럼의 고유 값은 통계 정보를 통해 확인할 수 있다.
...
#. 데이터 정합성을 위해 꼭 동기화 적용이 필요하지 않는 REDO 복구에 대해 병렬 스레드를 이용한 데이터 페이지별 복구를 적용하여 복구 성능 개선.
...
기존에 단일 스레드로 동작하던 unloaddb 유틸리티를 멀티 스레드 방식으로 개선하여, 데이터 추출 시간을 크게 단축할 수 있도록 기능을 향상
...
LIMIT 절에 바인드 변수와 계산식이 포함된 경우에도 Sort-Limit 최적화가 적용되도록 기능을 개선함.
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
데이터 삭제 시 통계 정보의 객체 수가 업데이트 되지 않아, 실행 계획 생성 시 부정확한 통계 정보가 사용 되어 성능에 영향을 줄 수 있는 문제를 개선함
...
ACL을 변경한 후 cubrid broker acl reload 명령어를 통해 재로드하면, 새로운 연결에 대해서는 변경된 ACL 규칙이 적용되지만, 기존에 연결된 CAS(CUBRID Application Server) 세션에는 변경 사항이 반영되지 않아 이전 규칙이 계속 유지되는 문제가 있었다. 
이에 따라, ACL 재로드 이후 이미 연결된 CAS 세션에 대해서도 트랜잭션이 완료된 시점부터 변경된 ACL 규칙을 적용하여 접근 제어가 이루어지도록 수정하였다.
