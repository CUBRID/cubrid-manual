...
CUBRID 11.4은 새로운 기능, 주요 변경 사항, 개선 사항 및 이전 버전에서 발생한 다수의 오류 수정이 포함된 최신 안정화 버전이다.
...
- 서버 프로세스의 메모리 관리 모듈을 사용하기 위해서는 시스템 파라미터 **enable_memory_monitoring** 가 yes로 설정한 후, 서버를 재 구동이 필요함.
...
- **[%broker1]** 은 클라이언트의 접속 정보가 미 작성되어 있으므로 기본 정책인 **DENY**\에 의헤 모든 클라이언트에 대해 접속을 허용하지 않음
...
- 운영 서버에서 백업본으로 데이터 복구가 필요할 때, 추가 계정 생성 및 추가 엔진 설치 등의 번거로운 작업이 없어 간단히 새로운 DB로 복구할 수 있는 ``rename_to_newdb.sh``  스크립트 지원. 자세한 사용 방법 및 내용은 메뉴얼 참조.
...
- 신규 컬럼을 추가해서 REBUILD시 신규 컬럼을 추가할 수 없다는 오류 발생하도록 변경
...
``ha_mode`` 가 **on** 이고 **ha_node_list** 에 **myhost** 가 있는 경우 오류 발생하도록 변경
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
- 상관 하위 질의 수행은 주 질의의 각 행에 대해 개별적으로 하위 질의가 수행되는데, 동일 조건값이 많은 경우 동일한 하위 질의가 반복 수행되어 성능 저하를 유발할 수 있는데, 캐시 적용하여 동일 조건값의 하위 질의가 반복 수행되는 것을 방지하여 성능 개선함.
...
JNI 프로그램이 세그폴트 멘트 오류 발생시 자동으로 javasp 서버를 재 구동하게 개선
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

여러 종류의 문자 인코딩(euckr, utf8)을 지원하도록 문자열 기능 개선
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

- 저장 프로시저의 문자열 인코딩 방식을 byte 배열로 처리해서 utf8 인코딩 이외에 euckr 인코등도 지원하도록 개선
...
- 특정 질의에서 LOCK이 생성되는 시점이 너무 이르기 때문에 불필요한 LOCK일 발생하고 있어서, 모든 조건들이 평가된 후에 불필요한 LOCK을 해제하도록 개선
...
- ``set trace on`` 상태에서 쿼리를 실행할 때 발생하던 **TRACE 통계 정보 수집**\의 성능 저하 문제를 해결하기 위해 *고비용 작업을 줄여*\,  ``set trace off`` 상태와 동일한 성능으로 실행할 수 있도록 개선되었습니다.
...
- ``diagdb`` Utility 사용에서 각 볼륨의 생성 시간을 확인할 수 있게 개선
- 로그 볼륨 헤더에 볼륨 생성 시간이 새롭게 추가됨에 따라 applyinfo 유틸리티 사용 시 볼륨 생성 시간을 추가로 출력하도록 수정

  - 로그 볼륨 헤더에 볼륨 **생성시간** ( ``Vol creation time`` ) 정보가 추가됨에 따라 HA환경에서 **활성 로그볼륨** 과 **보관 로그볼륨**  정보를 출력하는 ``applyinfo`` 유틸리티 또한 해당 볼륨 **생성시간** 정보를 추가로 출력하도록 수정
...
질의 최적화 중 조인 관계가 없는 테이블이 조인 테이블도 인식되는 오류 수정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
일반 사용자가 ``unloaddb`` 실행시, 트리거의 condition과 action_definition의 일반 사용지외 동일한 스키마명이 제거되지 않는 오류 수정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
