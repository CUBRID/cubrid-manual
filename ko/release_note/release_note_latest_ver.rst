...
본 문서는 CUBRID 11.4(빌드번호 11.4.0.1778)에 관한 정보를 포함한다.
...
- 조인 처리 방식 중 하나인 조인 조건 칼럼을 기준으로 해시 테이블을 생성해서 조인 처리하는 HASH JOIN을 지원한다.
...
- ``call change_serial_owner()`` 메서드만으로 SERIAL 소유자를 변경 가능했던 것을 SQL문으로 소유자 변경할 수 있도록 ALTER SERIAL 구문을 확장 지원함.
...
- 권한이 없거나 변경 대상인 SERIAL이 없을 경우에는 오류가 발생함.

생성된 그룹에 구성원을 추가/삭제할 수 있는 SQL 구문 지원
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

- *call add_member()*\와 *call drop_member()* 메서드만을 통해 생성된 그룹에 사용자를 추가/삭제할 수 있던 것을 SQL문으로 사용자를 추가/삭제할 수 있도록 ALTER USER 구문을 확장 지원함.

  - HA 환경에서 메서드 호출 통헤 생성된 그룹에 사용자를 추가/삭제하는 경우, 마스터와 슬레이브에서 별도 작업이 필요했던 작업을 ALTER USER 구문을 통해 쉽게 처리할 수 있도록 개선됨.
...
- 전체 질의 단위만으로만 사용할 수 있던 질의 캐시 기능을 부질의 단위까지 사용할 수 있도록 확장 지원함.
...
- trace 정보에 퀴리 캐시(*RESULT CACHE*) 사용 여부 표시 (reference count : 질의 캐시 참조 횟수)
...
- 메모리 모니터링 유틸리티인 cubrid memmon는 서버 프로세스(cub_server)의 메모리 관리 모듈로 부터 다음과 같은 힙 메모리 정보를 얻어 출력함.
   ...
   - 메모리를 할당 요청한 소스 코드와 라인 정보 기준으로 세부적인 메모리 할당량 정보
...
- databases.txt의 DB의 LOB 디렉터리의 위치의 변경시, 반드시 변경전 LOB 디렉터리에 존재하는 LOB 파일들을 변경 후 LOB 디렉터리로 이동 또는 복사 필요
...
테이블 생성 또는 칼럼 속성 변경시 ``AUTO_INCREMENT`` 와 ``DEFAULT`` 값이 공존할 수 없도록 변경
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

- **CREATE TABLE** 및 **ALTER COLUMN**  구문 수행 시 ``AUTO_INCREMENT`` 및 ``DEFAULT`` 속성을 함께 사용시 오류 발생하도록 변경

**ALTER INDEX … REBUILD**\에 신규 칼럼을 추가시 오류 발생하도록 변경
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

- 신규 칼럼을 추가한 후 REBUILD시 신규 칼럼을 추가할 수 없다는 오류가 발생하도록 변경
...
시스템 테이블 중 _db_serial의 칼럼명 변경 (att_name → attr_name)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

- 11.4 이전 버전에서 해당 칼럼명을 명시적으로 사용한 검색 질의는 11.4 버전에서 오류 발생하므로 수정 필요
...
상관 부질의 결과에 캐시 도입으로 성능 개선
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

- 상관 부질의는 주 질의의 각 행에 대해 개별적으로 수행된다. 이 과정에서 동일한 조건값이 많을 경우, 동일한 부질의가 반복적으로 수행되어 성능 저하를 유발할 수 있다. 이를 방지하기 위해 캐시를 적용하여, 동일 조건값의 부질의가 반복 수행되지 않도록 최적화하여 성능을 개선함.
...
다중 칼럼 인덱스에서 레코드를 읽을 때 필요한 midxkey.buf 크기 계산 방식을 개선하여 인덱스 스캔 성능 향상
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

- 기존에는 다중 칼럼 인덱스에서 키 길이 계산을 반복적으로 수행했던 것을 각 칼럼의 **OFFSET** 을 추가하여 계산 없이 직접 참조하도록 개선하여 **바이너리 검색, 키 필터링, DML 실행 시** 성능 향상.
...
- 인덱스 칼럼 ID를 반복적으로 비교하는 작업을 줄여 연산 최적화.
...
- 개선 사항
   ...
   - 인덱스 스캔 가능
   - 불 필요한 조인 제거
   - **상관 부질의**\에서 저장 프로시저 사용시 **결과 캐시** 사용 가능
...
**ha_node_list** 및 **ha_replica_list**\의 잘 못된 설정에 대해 명확한 오류 메시지 출력으로 사용자 인지 개선
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Failover와 Failback 발생에 대해 명확한 오류 메시지 출력으로 사용자 인지 개선
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Salve 노드에서 **restoreslave** 실행시 잘 못된 데이터 불일치 오류 메시지가 발생하지 않도록 개선
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
TRACE 활성화 시 질의 성능 개선
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

- ``set trace on`` 상태에서 질의를 실행할 때 발생하던 **TRACE 통계 정보 수집**\의 성능 저하 문제를 해결하기 위해 *고비용 작업을 줄여*\,  ``set trace off`` 상태와 동일한 성능으로 실행할 수 있도록 개선

문자열 유형에 대한 불필요한 길이 검사 제거를 통해 성능 개선
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

**lock_escalation** 설정 값 만큼만 락 자원을 유지하도록 변경해서 메모리 사용량 개선
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
ORDER BY 절이 있는 스칼라 부질의에서 hidden 칼럼 처리 오류 수정 
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

ORDER BY를 포함하는 인라인 뷰와 스칼라 부질의의 뷰 머징 처리시 ROWNUM이 ``ORDER_BY_NUM()``\으로 잘못 재 작성되는 오류 수정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
질의 최적화 중 INNER JOIN 제거 과정에서 테이블 위치가 잘못 처리되는 오류 수정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
**ORDERBY_NUM()**\이 포함된 부질의가 뷰 머징에 잘 못된 결과를 출력하는 오류 수정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

질의 최적화 중 조인 관계가 없는 테이블이 조인 테이블로 인식되는 오류 수정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

WHERE 절의 동등 조건(=)과 ORDER BY 절에서 사용된 칼럼이 동일할 때 ROWNUM 값이 0으로 출력되는 오류 수정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
뷰에 분석 함수가 포함될 때, 분석 함수 내부의 칼럼 순서 번호가 잘못되어 잘못된 결과를 출력하는 오류 수정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
JavaSP의 DATETIME 파라미터에 DATETIMELTZ 값을 시용시 발생한 오류 수정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

JavaSP의 선언된 Java 메서드명과 여는 괄호 사이의 공백으로 인해 JavaSP 수행되지 않는 오류 수정 
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

JavaSP의 미지원 타입을 보유한 테이블의 다른 칼럼을 인자로 사용시 발생한 오류 수정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
11.4 이전 버전의 ``unloaddb`` 파일을 loaddb 실행 시, 트리거 시스템 테이블 칼럼 (condition, action_definition)에 스키마명이 잘못 저장되거나 누락되는 오류 수정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

``unloaddb`` 실행 시 Reverse Unique Index의 주석이 누락되는 오류 수정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

``unloaddb`` 실행 시 auto_increment 설정과 함께 PK로 선언된 칼럼의 auto_increment 값이 1로 초기화되는 오류 수정
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
ALTER TABLE 구문에서 AUTO_INCREMENT 속성 갖는 칼럼 또는 default 값을 가지고 있는 칼럼의 타입 변경시, AUTO_INCREMENT 속성을 사용할 수 없는 타입으로 변경하거나 기존 default 값을 변환할 수 없는 타입으로 변경할 때, 오류가 발생함  (:ref:`change-column` 참조)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
USE INDEX(USING INDEX) 구문에서 유효하지 않은 인덱스명이나 테이블명을 지정하는 경우 오류 처리하지 않고 무시함  (해당 사항을 log 파일에 남기려면 error_log_warning 설정값을 yes로 설정하면 됨)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Alter change/modify 시 기입하지 않은 칼럼 속성은 유지되게 변경되었으며, alter 구문으로 auto_increment, on update property를 제거할 수 없게 변경됨 (:ref:`change-column` 참조)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

조건절에 칼럼명만 존재하는 경우 오류 처리되도록 변경됨
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
CUBRID 패키지내의 CCI Driver 디렉터리가 $CUBRID/lib, $CUBRID/include에서 $CUBRID/cci/lib, $CUBRID/cci/include로 각각 변경됨 (:ref:`CCI 개요 <cci-overview>` 참조)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
