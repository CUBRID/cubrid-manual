...
.. note:: **기존의 키 파일을 사용하는 데이터베이스 생성**
	...
	기존에 관리하던 키 파일이 있어 새로운 데이터베이스를 생성할 때에 이를 사용하고 싶다면, **tde_keys_file_path** 시스템 파라미터를 통하여 해당 키 파일의 경로를 설정한 후 데이터베이스를 생성하면 된다. 해당 시스템 파라미터에 대한 자세한 내용은 :ref:`other-parameters` 를 참고한다.
