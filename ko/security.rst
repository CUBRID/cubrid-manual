...
큐브리드는 브로커(broker)와 데이터베이스 서버의 2단계에서 특정 데이터베이스에 인가된 사용자 또는 응용프로그램에만 접근할 수 있는 기능을 지원한다.
브로커의 접근 제어는 WEB, WAS 등과 같은 응용프로그램들의 접근을 제어하는데 주로 사용하고, 데이터베이스 서버의 접근 제어는 브로커 및 csql 인터프리터등의 접근을 제어하는데 주로 사용한다.
...
큐브리드는 사용자(그룹)를 생성할 수 있고, 사용자가 생성한 테이블에 대해 다른 사용자(그룹)가 접근 여부를 제어할 수 있는 기능을 제공한다. 

자신이 만든 테이블에 다른 사용자(그룹)의 접근을 허용하려면 :ref:`granting-authorization` 구문을 사용하여 해당 사용자(그룹)에게 적절한 권한을 제공할 수 있고, 접근 권한을 해지하기 위해서는 :ref:`revoking-authorization` 구문을 사용하여 해당 사용자(그룹)으로부터 권한을 회수할 수 있다. PUBLIC 사용자 생성한 (가상) 테이블에 대해서는 권한 제공 절차 없이 모든 사용자에게 접근이 허용된다.
