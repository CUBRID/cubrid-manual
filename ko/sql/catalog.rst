...
|                         |                           | 0: NONE, 1: AES, 2: ARIA                     |
+-------------------------+---------------------------+----------------------------------------------+
| statistics_strategy     | INTEGER                   | 통계 정보 수집 전략                          |
|                         |                           | (0: SAMPLING, 1: FULLSCAN)                   |
+-------------------------+---------------------------+----------------------------------------------+
| flags                   | INTEGER                   | 클래스 플래그                                |
|                         |                           |                                              |
|                         |                           | - **Bit 1**: WITH CHECK OPTION 속성이        |
|                         |                           |   있으면 1                                   |
|                         |                           | - **Bit 2**: LOCAL CHECK OPTION 속성이       |
|                         |                           |   있으면 1                                   |
|                         |                           | - **Bit 3**: REUSE_OID 클래스인 경우 1       |
|                         |                           | - 나머지 비트는 현재 사용하지 않는다.        |
+-------------------------+---------------------------+----------------------------------------------+
...
|                         |                           |                                              |
+-------------------------+---------------------------+----------------------------------------------+
| meth_files              | SEQUENCE OF _db_meth_file | 메서드에 대한 함수가 위치한 파일 경로        |
|                         |                           |                                              |
+-------------------------+---------------------------+----------------------------------------------+
| query_specs             | SEQUENCE OF _db_query_spec| 가상 클래스인 경우 그 SQL 정의문             |
|                         |                           |                                              |
+-------------------------+---------------------------+----------------------------------------------+
...
|                         |                           |                                              |
+-------------------------+---------------------------+----------------------------------------------+
| created_time            | DATETIME                  | 클래스 생성 시각                             |
+-------------------------+---------------------------+----------------------------------------------+
| updated_time            | DATETIME                  | 클래스 수정 시각                             |
+-------------------------+---------------------------+----------------------------------------------+
| checked_time            | DATETIME                  | 클래스의 통계 정보 갱신 시각                 |
+-------------------------+---------------------------+----------------------------------------------+
...
+----------------+------------------------+-----------------------------------------------------------------------------------------------------------------+
| comment        | VARCHAR(2048)          | 속성에 대한 설명.                                                                                               |
+----------------+------------------------+-----------------------------------------------------------------------------------------------------------------+
...
도메인에 대한 정보이며, object_of와 data_type 칼럼에 각각(개별적인) 인덱스가 생성되어 있다.
...
|             |                                            |                                                               |
+-------------+--------------------------------------------+---------------------------------------------------------------+
| enumeration | SEQUENCE OF CHARACTER VARYING(1073741823)  | 열거형 타입 정의 문자열                                       |
+-------------+--------------------------------------------+---------------------------------------------------------------+
| set_domains | SEQUENCE OF _db_domain                     | 컬렉션 타입인 경우, 그 집합을 구성하는 원소의 데이터 타입에   |
|             |                                            | 대한 도메인 정보. 컬렉션 타입이 아닌 경우 **NULL**            |
|             |                                            | 이 설정됨                                                     |
+-------------+--------------------------------------------+---------------------------------------------------------------+
| json_schema | CHARACTER VARYING(1073741823)              | JSON 타입인 경우, JSON 데이터 구조를 정의하는 스키마 정보.    |
|             |                                            | JSON 타입이 아닌 경우 **NULL** 이 설정됨                      |
+-------------+--------------------------------------------+---------------------------------------------------------------+
...
+--------------+--------------------------+-------------------------+
| return_value | SEQUENCE OF _db_meth_arg | 함수의 반환 값          |
+--------------+--------------------------+-------------------------+
...
|             |                        | 의 "CUBRID가 지원하는 데이터 타입" 표의 "값" 중 하나)     |
+-------------+------------------------+-----------------------------------------------------------+
| index_of    | INTEGER                | 함수정의에 인자가 나열된 순서. 반환 값이면 0,             |
|             |                        | 입력인자이면 1부터 시작함.                                |
+-------------+------------------------+-----------------------------------------------------------+
...
+------------------+---------------------+---------------------------------+------------------------+
| invalidated_time | DATETIME            | 뷰 질의가 무효화된 시각         |                        |
+------------------+---------------------+---------------------------------+------------------------+
...
+--------------------------+---------------------------+----------------------------------------------------------------------+
| filter_expression        | VARCHAR(1073741823)       | 필터링된 인덱스의 조건                                               |
+--------------------------+---------------------------+----------------------------------------------------------------------+
| have_function            | INTEGER                   | 함수 기반 인덱스이면 1, 그렇지 않으면 0                              |
+--------------------------+---------------------------+----------------------------------------------------------------------+
| status                   | INTEGER                   | 인덱스 상태                                                          |
+--------------------------+---------------------------+----------------------------------------------------------------------+
| referential_index        | _db_index                 | 외래 키가 참조하는 인덱스 (외래 키가 아닌 경우 NULL)                 |
+--------------------------+---------------------------+----------------------------------------------------------------------+
| delete_rule              | INTEGER                   | 외래 키 제약 조건의 삭제 규칙                                        |
|                          |                           | (0: CASCADE, 1: RESTRICT, 2: NO ACTION, 3: SET NULL)                 |
+--------------------------+---------------------------+----------------------------------------------------------------------+
| update_rule              | INTEGER                   | 외래 키 제약 조건의 업데이트 규칙                                    |
|                          |                           | (1: RESTRICT, 2: NO ACTION, 3: SET NULL)                             |
+--------------------------+---------------------------+----------------------------------------------------------------------+
| referential_match_option | INTEGER                   | 외래키 제약 조건의 일치 옵션 (0: NONE)                               |
|                          |                           |                                                                      |
|                          |                           | - NONE: NULL이 하나라도 존재 시 검사하지 않음                        |
+--------------------------+---------------------------+----------------------------------------------------------------------+
| index_type               | INTEGER                   | 인덱스 타입 (0: BTREE)                                               |
+--------------------------+---------------------------+----------------------------------------------------------------------+
| options                  | INTEGER                   | 인덱스 옵션                                                          |
|                          |                           |                                                                      |
|                          |                           | - **Bit 0 ~ Bit 3**: 인덱스의 **deduplicate level** 을 나타낸다.     |
|                          |                           |   자세한 내용은 :ref:`deduplicate_overview` 을 참고한다.             |
|                          |                           | - 나머지 비트는 현재 사용하지 않는다.                                |
+--------------------------+---------------------------+----------------------------------------------------------------------+
| comment                  | VARCHAR(1024)             | 인덱스 설명                                                          |
+--------------------------+---------------------------+----------------------------------------------------------------------+
| created_time             | DATETIME                  | 인덱스 생성 시각                                                     |
+--------------------------+---------------------------+----------------------------------------------------------------------+
| updated_time             | DATETIME                  | 인덱스 수정 시각                                                     |
+--------------------------+---------------------------+----------------------------------------------------------------------+
...
객체에 대한 사용자 권한 정보를 표현하며, grantee에 대한 인덱스와 object_of에 대한 인덱스가 생성되어 있다.

+--------------+-------------+-----------------------------------------------------------------------------+
| 속성명       | 데이터 타입 | 설명                                                                        |
+==============+=============+=============================================================================+
| grantor      | _db_user    | 권한 부여자                                                                 |
+--------------+-------------+-----------------------------------------------------------------------------+
| grantee      | _db_user    | 권한 피부여자                                                               |
+--------------+-------------+-----------------------------------------------------------------------------+
| object_type  | INTEGER     | 권한부여 대상인 객체 타입 (0: 클래스, 5: 저장 프로시저)                     |
+--------------+-------------+-----------------------------------------------------------------------------+
| object_of    | object      | 권한부여 대상인 객체                                                        |
+--------------+-------------+-----------------------------------------------------------------------------+
| auth_type    | VARCHAR(7)  | 부여된 권한 타입 이름                                                       |
+--------------+-------------+-----------------------------------------------------------------------------+
| is_grantable | INTEGER     | 권한 받은 객체에 대해 다른 사용자에게 권한을 부여할 수 있으면 1, 아니면 0   |
+--------------+-------------+-----------------------------------------------------------------------------+
| created_time | DATETIME    | 권한 부여 시각                                                              |
+--------------+-------------+-----------------------------------------------------------------------------+
| updated_time | DATETIME    | 권한 변경 시각                                                              |
+--------------+-------------+-----------------------------------------------------------------------------+
...
다음 예제에서는 뷰 *db_trigger* 에 정의되어 있는 권한 정보를 검색한다.
...
+-----------+-------------+------------------------------------------------------------------------+
| type_name | VARCHAR(16) | 데이터 타입 이름. 'CUBRID가 지원하는 데이터 타입' 표의 '의미'에 해당함 |
+-----------+-------------+------------------------------------------------------------------------+
...
|                      |               | - 2 - LIST                       |
+----------------------+---------------+----------------------------------+
| pexpr                | VARCHAR(2048) | Parent 에만 해당                 |
+----------------------+---------------+----------------------------------+
| pvalues              | SEQUENCE OF   | - Hash - 칼럼명, Hash size       |
|                      |               | - RANGE - MIN/MAX value -        |
|                      |               |   무한의 MIN/MAX는 **NULL** 로   |
|                      |               |   저장                           |
|                      |               | - LIST - value list              |
+----------------------+---------------+----------------------------------+
| class_partition_type | INTEGER       | 파티션 클래스의 파티션 타입      |
|                      |               | (1: PARTITIONED CLASS,           |
|                      |               | 2: PARTITION CLASS)              |
|                      |               |                                  |
|                      |               | - PARTITIONED CLASS: 파티션을    |
|                      |               |   가지는 부모 테이블             |
|                      |               | - PARTITION CLASS: 실제 데이터를 |
|                      |               |   저장하는 개별 파티션 테이블    |
+----------------------+---------------+----------------------------------+
...
return_type          INTEGER                               반환 값 타입
arg_count            INTEGER                               파라미터 개수
args                 SEQUENCE OF _db_stored_procedure_args 파라미터 리스트
lang                 INTEGER                               구현 언어
...
owner                _db_user                              저장 프로시저의 소유자
comment              VARCHAR(1024)                         저장 프로시저 설명
created_time         DATETIME                              저장 프로시저 생성 시각
updated_time         DATETIME                              저장 프로시저 수정 시각
==================== ===================================== =========================================================
...
index_of             INTEGER                     파라미터의 순서
is_system_generated  INTEGER                     시스템이 생성한 저장 프로시저 인수 여부
arg_name             CHARACTER VARYING(255)      파라미터의 이름
data_type            INTEGER                     파라미터의 데이터 타입
mode                 INTEGER                     파라미터 모드 (IN, OUT, INOUT)
default_value        CHARACTER VARYING(255)      파라미터의 기본값
is_optional          INTEGER                     선택적 파라미터 여부
comment              CHARACTER VARYING(1024)     저장 프로시저 인자 설명
==================== =========================== =========================================================

        .. note::

            - **index_of**: 파라미터의 순서는 0부터 시작한다.

            - **is_system_generated**: 값이 **1** 이면 시스템이 생성한 저장 프로시저 인수이며, **0** 이면 사용자가 생성한 저장 프로시저 인수이다.

            - **data_type**: 파라미터의 데이터 타입은 아래 값 중 하나이다
            ...
            - **mode**: 파라미터 모드는 아래 값 중 하나이다
                
                ::
                
                        IN = 0, OUT = 1, INOUT = 2

            - **default_value**: 파라미터의 기본값이다. 자세한 내용은 :ref:`pl-arg-default` 을 참고한다.

            - **is_optional**: 파라미터가 선택 가능하면 **1**,  필수이면 **0** 이다.
...
created_time         CHARACTER VARYING(16)         저장 프로시저 코드의 생성 시각
owner                _db_user                      저장 프로시저 코드의 소유자
...
owner            _db_user            연결 정보의 소유자
comment          VARCHAR(1024)       서버 설명
created_time     DATETIME            서버 생성 시각
updated_time     DATETIME            서버 수정 시각
invalidated_time DATETIME            외부 서버 연결이 무효화된 시각
================ =================== ====================================================
...
target_owner       _db_user      대상 객체 소유자
comment            VARCHAR(2048) 동의어 설명
created_time       DATETIME      동의어 생성 시각
updated_time       DATETIME      동의어 수정 시각
================== ============= =========================================================
...
사용자에 대한 정보를 표현하며 name에 대한 인덱스가 생성되어 있다.

+--------------------+---------------------+-----------------------------------------------------------------------------------+
|   속성명           |   데이터 타입       |   설명                                                                            |
+====================+=====================+===================================================================================+
| name               | VARCHAR(32)         | 사용자명                                                                          |
+--------------------+---------------------+-----------------------------------------------------------------------------------+
| id                 | INTEGER             | 사용자 식별자                                                                     |
+--------------------+---------------------+-----------------------------------------------------------------------------------+
| password           | _db_password        | 사용자 패스워드. 사용자에게는 표시되지 않는다.                                    |
+--------------------+---------------------+-----------------------------------------------------------------------------------+
| direct_groups      | SET OF _db_user     | 사용자가 직접적으로 속한 그룹                                                     |
+--------------------+---------------------+-----------------------------------------------------------------------------------+
| groups             | SET OF _db_user     | 사용자가 직·간접적으로 속한 그룹                                                  |
+--------------------+---------------------+-----------------------------------------------------------------------------------+
| authorization      | _db_authorization   | 사용자가 소유한 권한 정보                                                         |
+--------------------+---------------------+-----------------------------------------------------------------------------------+
| triggers           | SEQUENCE OF object  | 사용자의 action에 의해 발생하는 트리거들                                          |
+--------------------+---------------------+-----------------------------------------------------------------------------------+
| is_loginable       | INTEGER             | 로그인 허용이면 1, 비허용이면 0                                                   |
+--------------------+---------------------+-----------------------------------------------------------------------------------+
| is_system_created  | INTEGER             | 시스템이 생성한 사용자이면 1 (예: **DBA**, **PUBLIC**), 아니면 0                  |
+--------------------+---------------------+-----------------------------------------------------------------------------------+
| comment            | VARCHAR(1024)       | 사용자 설명                                                                       |
+--------------------+---------------------+-----------------------------------------------------------------------------------+
| created_time       | DATETIME            | 사용자 생성 시각                                                                  |
+--------------------+---------------------+-----------------------------------------------------------------------------------+
| updated_time       | DATETIME            | 사용자 수정 시각                                                                  |
+--------------------+---------------------+-----------------------------------------------------------------------------------+
...
|   속성명           |   데이터 타입      |   설명                                                                                                                           |
+====================+====================+==================================================================================================================================+
| owner              | _db_user           | 이 권한 객체의 소유자                                                                                                            |
+--------------------+--------------------+----------------------------------------------------------------------------------------------------------------------------------+
| grants             | SEQUENCE           | {권한이 부여된 객체, 객체의 권한 부여자, 권한 종류}의 sequence                                                                   |
+--------------------+--------------------+----------------------------------------------------------------------------------------------------------------------------------+
...
시리얼에 대한 정보를 표현하며 unique_name에 대한 인덱스와 name, owner에 대한 인덱스가 생성되어 있다.
...
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| start_val      | NUMERIC(38,0)        | 시리얼의 시작값. 기본값은 1                                                                         |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
...
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| attr_name      | VARCHAR(1073741823)  | AUTO_INCREMENT 시리얼이면 칼럼명을 저장. 아니면 **NULL**                                            |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| cached_num     | INTEGER              | 성능 향상을 위해 메모리에 미리 생성할 시리얼 값의 개수. 기본값은 0                                  |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| comment        | VARCHAR(1024)        | 시리얼 설명                                                                                         |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| created_time   | DATETIME             | 시리얼 생성 시각                                                                                    |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| updated_time   | DATETIME             | 시리얼 수정 시각                                                                                    |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
...
+------------------------+---------------------+--------------------------------------------------------------------------------------------------------+
| created_time           | DATETIME            | 트리거 생성 시각                                                                                       |
+------------------------+---------------------+--------------------------------------------------------------------------------------------------------+
| updated_time           | DATETIME            | 트리거 수정 시각                                                                                       |
+------------------------+---------------------+--------------------------------------------------------------------------------------------------------+
...
|                      |               | 생성 시각                                                                   |
+----------------------+---------------+-----------------------------------------------------------------------------+
| log_commit_time      | DATETIME      | 마지막 commit log의 반영 시각                                               |
+----------------------+---------------+-----------------------------------------------------------------------------+
| last_access_time     | DATETIME      | _db_ha_apply_info 카탈로그의 최종 갱신 시각                                 |
+----------------------+---------------+-----------------------------------------------------------------------------+
...
+----------------------+---------------+-----------------------------------------------------------------------------+
| start_time           | DATETIME      | applylogdb 프로세스가 슬레이브 DB에 접속한 시각                             |
+----------------------+---------------+-----------------------------------------------------------------------------+
...
| class_name         | VARCHAR(255)  | 클래스명                                     |
+--------------------+---------------+----------------------------------------------+
| owner_name         | VARCHAR(32)   | 클래스 소유자명                              |
+--------------------+---------------+----------------------------------------------+
...
+--------------------+---------------+----------------------------------------------+
| statistics_strategy| VARCHAR(8)    | 통계 정보 수집 전략                          |
|                    |               | ('SAMPLING', 'FULLSCAN')                     |
+--------------------+---------------+----------------------------------------------+
...
+------------------+---------------+------------------------------+
| super_owner_name | VARCHAR(32)   | 한 단계 상위 클래스 소유자명 |
+------------------+---------------+------------------------------+
...
+------------------+----------------------+----------------------------------+------------------------+
| owner_name       | VARCHAR(32)          | 가상 클래스 소유자명             |                        |
+------------------+----------------------+----------------------------------+------------------------+
|                  | VARCHAR(1073741823)  |                                  | 10.1 Patch 4 이후      |
...
+------------------+----------------------+----------------------------------+------------------------+
| created_time     | DATETIME             | 가상 클래스 생성 시각            |                        |
+------------------+----------------------+----------------------------------+------------------------+
| updated_time     | DATETIME             | 가상 클래스 수정 시각            |                        |
+------------------+----------------------+----------------------------------+------------------------+
| invalidated_time | DATETIME             | 뷰 질의가 무효화된 시각          |                        |
+------------------+----------------------+----------------------------------+------------------------+
...
+-------------------+---------------+---------------------------------------------------------------------------------------------------------------+
| owner_name        | VARCHAR(32)   | 속성이 속한 클래스 소유자명                                                                                   |
+-------------------+---------------+---------------------------------------------------------------------------------------------------------------+
...
+-------------------+---------------+---------------------------------------------------------------------------------------------------------------+
| from_owner_name   | VARCHAR(32)   | 상속받은 속성이면 그 속성이 정의되어 있는 상위 클래스의 소유자명이 설정되며, 그렇지 않으면 **NULL**           |
+-------------------+---------------+---------------------------------------------------------------------------------------------------------------+
...
+-------------------+---------------+---------------------------------------------------------------------------------------------------------------+
| domain_owner_name | VARCHAR(32)   | 데이터 타입이 객체 타입인 경우 그 도메인 클래스 소유자명. 객체 타입이 아닌 경우 **NULL**                      |
+-------------------+---------------+---------------------------------------------------------------------------------------------------------------+
...
+-------------------+---------------+-------------------------------------------------------------------------------+
| domain_owner_name | VARCHAR(32)   | 원소의 데이터 타입이 객체 타입인 경우 그 도메인 클래스 소유자명               |
+-------------------+---------------+-------------------------------------------------------------------------------+
...
| coll_name      | VARCHAR(32)  | 콜레이션 이름                                                                 |
+----------------+--------------+-------------------------------------------------------------------------------+
| charset_name   | VARCHAR(32)  | 문자셋 이름                                                                   |
+----------------+--------------+-------------------------------------------------------------------------------+
...
+-----------------+---------------+-------------------------------------------------------------------------------------+
| owner_name      | VARCHAR(32)   | 메서드가 속한 클래스 소유자명                                                       |
+-----------------+---------------+-------------------------------------------------------------------------------------+
...
|                 |               | 않으면 **NULL**                                                                     |
+-----------------+---------------+-------------------------------------------------------------------------------------+
| from_owner_name | VARCHAR(32)   | 상속받은 메서드이면 그 메서드가 정의되어 있는 상위 클래스 소유자명이 설정되며       |
|                 |               | 그렇지 않으면 **NULL**                                                              |
+-----------------+---------------+-------------------------------------------------------------------------------------+
| from_meth_name  | VARCHAR(255)  | 상속받은 메서드이며, 이름 충돌이 발생하여 이를 해결하기 위해 그 메서드명이 바뀐     |
|                 |               | 경우, 상위 클래스에 정의된 원래 이름이 설정됨. 그 이외에는 모두 **NULL**            |
...
다음 예제에서는 클래스 *_db_user* 의 메서드를 검색한다.
...
| meth_type         | VARCHAR(8)    | 인스턴스 메서드이면 'INSTANCE', 클래스 메서드이면 'CLASS'                |
+-------------------+---------------+--------------------------------------------------------------------------+
| index_of          | INTEGER       | 인자가 함수 정의에 나열된 순서. 반환 값이면 0, 입력인자이면 1부터 시작함 |
+-------------------+---------------+--------------------------------------------------------------------------+
...
+-------------------+---------------+--------------------------------------------------------------------------+
| domain_owner_name | VARCHAR(32)   | 인자의 데이터 타입이 객체 타입인 경우 도메인 클래스 소유자명             |
+-------------------+---------------+--------------------------------------------------------------------------+

다음 예제에서는 클래스 *_db_user* 의 메서드 입력 인자를 검색한다.
...
+-------------------+--------------+--------------------------------------------------------------------------+
| domain_owner_name | VARCHAR(32)  | 원소의 데이터 타입이 객체 타입인 경우 도메인 클래스 소유자명             |
+-------------------+--------------+--------------------------------------------------------------------------+
...
+-----------------+--------------+----------------------------------------------------------------------------------------+
| owner_name      | VARCHAR(32)  | 메서드 파일이 속한 클래스 소유자명                                                     |
+-----------------+--------------+----------------------------------------------------------------------------------------+
...
|                 |              | 그렇지 않으면 **NULL**                                                                 |
+-----------------+--------------+----------------------------------------------------------------------------------------+
| from_owner_name | VARCHAR(32)  | 상속받은 메서드이면 그 메서드 파일이 정의되어 있는 상위 클래스 소유자명이 설정.        |
|                 |              | 그렇지 않으면 **NULL**                                                                 |
+-----------------+--------------+----------------------------------------------------------------------------------------+
...
+------------------------------------+---------------------+---------------------------------------------------------+
| owner_name                         | VARCHAR(32)         | 인덱스가 속한 클래스 소유자명                           |
+------------------------------------+---------------------+---------------------------------------------------------+
| key_count                          | INTEGER             | 키를 구성하는 속성의 개수                               |
+------------------------------------+---------------------+---------------------------------------------------------+
...
+------------------------------------+---------------------+---------------------------------------------------------+
| status                             | VARCHAR(255)        | 인덱스 상태                                             |
+------------------------------------+---------------------+---------------------------------------------------------+
| referential_index_class_owner_name | VARCHAR(32)         | 외래 키가 참조하는 인덱스의 클래스 소유자명             |
+------------------------------------+---------------------+---------------------------------------------------------+
| referential_index_class_name       | VARCHAR(255)        | 외래 키가 참조하는 인덱스의 클래스명                    |
+------------------------------------+---------------------+---------------------------------------------------------+
| referential_index_name             | VARCHAR(255)        | 외래 키가 참조하는 인덱스명                             |
+------------------------------------+---------------------+---------------------------------------------------------+
| delete_rule                        | VARCHAR(32)         | 외래 키 제약 조건의 삭제 규칙                           |
|                                    |                     | (CASCADE, RESTRICT, NO ACTION, SET NULL)                |
+------------------------------------+---------------------+---------------------------------------------------------+
| update_rule                        | VARCHAR(32)         | 외래 키 제약 조건의 업데이트 규칙                       |
|                                    |                     | (RESTRICT, NO ACTION, SET NULL)                         |
+------------------------------------+---------------------+---------------------------------------------------------+
| referential_match_option           | VARCHAR(7)          | 외래키 제약 조건의 일치 옵션 (NONE)                     |
|                                    |                     |                                                         |
|                                    |                     | - NONE: NULL이 하나라도 존재 시 검사하지 않음           |
+------------------------------------+---------------------+---------------------------------------------------------+
| index_type                         | VARCHAR(32)         | 인덱스 타입 (BTREE)                                     |
+------------------------------------+---------------------+---------------------------------------------------------+
| deduplicate_key_level              | INTEGER             | 인덱스의 중복 제거 레벨                                 |
+------------------------------------+---------------------+---------------------------------------------------------+
| comment                            | VARCHAR(1024)       | 인덱스 설명                                             |
+------------------------------------+---------------------+---------------------------------------------------------+
| created_time                       | DATETIME            | 인덱스 생성 시각                                        |
...
데이터베이스 내에서 현재 사용자가 권한을 가지는 객체에 대한 권한 정보를 보여준다.

+--------------+--------------+------------------------------------------------------------------------------------+
| 속성명       | 데이터 타입  | 설명                                                                               |
+==============+==============+====================================================================================+
| grantor_name | VARCHAR(32)  | 권한을 부여한 사용자명                                                             |
+--------------+--------------+------------------------------------------------------------------------------------+
| grantee_name | VARCHAR(32)  | 권한을 받은 사용자명                                                               |
+--------------+--------------+------------------------------------------------------------------------------------+
| object_type  | VARCHAR(16)  | 권한부여 대상인 객체 타입명 (CLASS, VCLASS, PROCEDURE, FUNCTION)                   |
+--------------+--------------+------------------------------------------------------------------------------------+
| object_name  | VARCHAR(255) | 권한부여 대상인 객체명                                                             |
+--------------+--------------+------------------------------------------------------------------------------------+
| owner_name   | VARCHAR(32)  | 권한부여 대상인 객체 소유자명                                                      |
+--------------+--------------+------------------------------------------------------------------------------------+
| auth_type    | VARCHAR(7)   | 부여된 권한 타입명                                                                 |
+--------------+--------------+------------------------------------------------------------------------------------+
| is_grantable | VARCHAR(3)   | 권한 받은 객체에 대해 다른 사용자에게 권한을 부여할 수 있으면 'YES', 아니면 'NO'   |
+--------------+--------------+------------------------------------------------------------------------------------+
| created_time | DATETIME     | 권한 부여 시각                                                                     |
+--------------+--------------+------------------------------------------------------------------------------------+
| updated_time | DATETIME     | 권한 변경 시각                                                                     |
...
다음 예제에서는 이름이 *db_a* 로 시작되는 객체의 권한 정보를 검색한다.
...
데이터베이스 내에서 현재 사용자가 접근 권한을 가진 사용자에 대한 정보를 보여준다.
...
| id                 | INTEGER             | 사용자 식별자                                                                    |
+--------------------+---------------------+----------------------------------------------------------------------------------+
| password           | _db_password        | 항상 **NULL** (호환성을 위해 유지)                                               |
+--------------------+---------------------+----------------------------------------------------------------------------------+
| direct_groups      | SET OF VARCHAR(32)  | 사용자가 직접적으로 속한 그룹의 이름 집합                                        |
+--------------------+---------------------+----------------------------------------------------------------------------------+
| groups             | SET OF VARCHAR(32)  | 사용자가 직·간접적으로 속한 그룹의 이름 집합                                     |
+--------------------+---------------------+----------------------------------------------------------------------------------+
| authorization      | _db_authorization   | 항상 **NULL** (호환성을 위해 유지)                                               |
+--------------------+---------------------+----------------------------------------------------------------------------------+
| triggers           | SEQUENCE OF object  | 사용자의 action에 의해 발생하는 트리거들                                         |
+--------------------+---------------------+----------------------------------------------------------------------------------+
| is_loginable       | VARCHAR(3)          | 로그인 허용이면 'YES', 비허용이면 'NO'                                           |
+--------------------+---------------------+----------------------------------------------------------------------------------+
| is_system_created  | VARCHAR(3)          | 시스템 생성 사용자이면 'YES' (예: **DBA**, **PUBLIC**), 아니면 'NO'              |
+--------------------+---------------------+----------------------------------------------------------------------------------+
| comment            | VARCHAR(1024)       | 사용자 설명                                                                      |
+--------------------+---------------------+----------------------------------------------------------------------------------+
| created_time       | DATETIME            | 사용자 생성 시각                                                                 |
...
데이터베이스 내에서 현재 사용자가 접근 권한을 가진 클래스에 대한 사용자 권한 정보를 보여준다.

+--------------------+--------------------+-----------------------------------------------------------------------------------+
|   속성명           |   데이터 타입      |   설명                                                                            |
+====================+====================+===================================================================================+
| owner              | VARCHAR(32)        | 이 권한 객체의 소유자명                                                           |
+--------------------+--------------------+-----------------------------------------------------------------------------------+
| grants             | SEQUENCE           | {권한이 부여된 객체, 객체의 권한 부여자, 권한 종류}의 sequence                    |
+--------------------+--------------------+-----------------------------------------------------------------------------------+
...
데이터베이스 내에서 현재 사용자가 접근 권한을 가진 시리얼에 대한 정보를 보여준다.

+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| 속성명         | 데이터 타입          | 설명                                                                                                |
+================+======================+=====================================================================================================+
| unique_name    | VARCHAR(255)         | 스키마명을 포함하는 시리얼명                                                                        |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| name           | VARCHAR(255)         | 시리얼명                                                                                            |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| owner          | VARCHAR(32)          | 시리얼 소유자                                                                                       |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| current_val    | NUMERIC(38,0)        | 현재 시리얼 값                                                                                      |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| increment_val  | NUMERIC(38,0)        | 시리얼 값의 간격                                                                                    |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| max_val        | NUMERIC(38,0)        | 시리얼의 최대값                                                                                     |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| min_val        | NUMERIC(38,0)        | 시리얼의 최소값                                                                                     |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| start_val      | NUMERIC(38,0)        | 시리얼의 시작값                                                                                     |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| cyclic         | INTEGER              | 시리얼의 최대 또는 최소값에 도달한 후 순환해서 값을 생성할 수 있으면 1 (CYCLE), 없으면 0 (NOCYCLE)  |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| started        | INTEGER              | 생성 후 한 번이라도 값을 생성했으면 1, 아니면 0                                                     |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| class_name     | VARCHAR(255)         | AUTO_INCREMENT 시리얼이면 테이블명을 저장. 아니면 **NULL**                                          |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| attr_name      | VARCHAR(255)         | AUTO_INCREMENT 시리얼이면 칼럼명을 저장. 아니면 **NULL**                                            |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| cached_num     | INTEGER              | 성능 향상을 위해 메모리에 미리 생성할 시리얼 값의 개수.                                             |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| comment        | VARCHAR(1024)        | 시리얼 설명                                                                                         |
+----------------+----------------------+-----------------------------------------------------------------------------------------------------+
| created_time   | DATETIME             | 시리얼 생성 시각                                                                                    |
...
+-------------------+--------------+-----------------------------------------------------------------------------------------------------+
| owner_name        | VARCHAR(32)  | 트리거 소유자명                                                                                     |
+-------------------+--------------+-----------------------------------------------------------------------------------------------------+
| target_class_name | VARCHAR(255) | 대상이 되는 클래스명                                                                                |
+-------------------+--------------+-----------------------------------------------------------------------------------------------------+
| target_owner_name | VARCHAR(32)  | 대상이 되는 클래스 소유자명                                                                         |
+-------------------+--------------+-----------------------------------------------------------------------------------------------------+
...
데이터베이스 내에서 복제 로그 반영 과정과 그 진행 상태를 나타내는 정보를 보여준다.
...
+----------------------+---------------+------------------------------------------------------------------------------------+
| partition_expr       | VARCHAR(2048) | 파티션 표현식                                                                      |
+----------------------+---------------+------------------------------------------------------------------------------------+
| partition_values     | SEQUENCE OF   | 파티션 유형이 RANGE인 경우 MIN 및 MAX 값이고, MIN 및 MAX가 무한인 경우 **NULL**    |
|                      |               | 파티션 유형이 LIST인 경우 값 목록                                                  |
+----------------------+---------------+------------------------------------------------------------------------------------+
| class_partition_type | VARCHAR(32)   | 파티션 클래스의 파티션 타입 (PARTITION CLASS)                                      |
|                      |               |                                                                                    |
|                      |               | - PARTITION CLASS: 실제 데이터를 저장하는 개별 파티션 테이블                       |
+----------------------+---------------+------------------------------------------------------------------------------------+
| comment              | VARCHAR(1024) | 파티션 설명                                                                        |
+----------------------+---------------+------------------------------------------------------------------------------------+
| created_time         | DATETIME      | 클래스 생성 시각 (부모 클래스로부터 상속)                                          |
+----------------------+---------------+------------------------------------------------------------------------------------+
| updated_time         | DATETIME      | 클래스 수정 시각 (부모 클래스로부터 상속)                                          |
+----------------------+---------------+------------------------------------------------------------------------------------+
...
return_type          VARCHAR(16)                 저장 프로시저의 반환 타입 이름
arg_count            INTEGER                     파라미터 개수
...
==================== =========================== =========================================================
sp_name              VARCHAR(255)                저장 프로시저 이름
owner_name           VARCHAR(32)                 저장 프로시저 소유자의 이름
pkg_name             VARCHAR(255)                저장 프로시저가 속한 패키지 이름
index_of             INTEGER                     파라미터의 순서
arg_name             VARCHAR(255)                파라미터의 이름
data_type            VARCHAR(16)                 파라미터의 데이터 타입
mode                 VARCHAR(6)                  파라미터 모드 (IN, OUT, INOUT)
is_optional          VARCHAR(3)                  선택적 파라미터 여부
default_value        VARCHAR(255)                파라미터의 기본값
comment              VARCHAR(1024)               파라미터에 대한 설명
==================== =========================== =========================================================

        .. note::

            - **index_of**: 파라미터의 순서는 0부터 시작하고 63을 최대값으로 가질 수 있다.
            ...
            - **is_optional**: 파라미터가 선택 가능하면 **YES**,  필수이면 **NO** 이다.
...
user_name        VARCHAR(255)  서버의 데이터베이스 사용자 이름
properties       VARCHAR(2048) 연결 시 사용되는 프로퍼티 정보
owner            VARCHAR(32)   연결 정보의 소유자 이름
comment          VARCHAR(1024) 서버 설명
created_time     DATETIME      서버 생성 시각
updated_time     DATETIME      서버 수정 시각
invalidated_time DATETIME      외부 서버 연결이 무효화된 시각
...
synonym_owner_name VARCHAR(32)   동의어의 소유자 이름
is_public_synonym  VARCHAR(3)    공용(Public) 동의어이면 "YES"이고, 전용(Private) 동의어이면 "NO"
target_name        VARCHAR(255)  대상 객체 이름
target_owner_name  VARCHAR(32)   대상 객체의 소유자 이름
comment            VARCHAR(2048) 동의어 설명
created_time       DATETIME      동의어 생성 시각
updated_time       DATETIME      동의어 수정 시각
================== ============= ================================================================
...
카탈로그 클래스들은 **dba** 소유로 생성된다. 그러나, **dba** 가 **SELECT** 연산만 수행할 수 있을 뿐이며, **UPDATE** / **DELETE** 등의 연산을 수행할 경우에는 authorization failure 오류가 발생한다. 일반 사용자는 시스템 카탈로그 클래스에 대해서 질의를 수행할 수 없다.
...
클래스, 가상 클래스, 속성, 트리거, 메서드, 인덱스 이름 등과 같은 식별자(identifier)는 모두 소문자로 변경되어 시스템 카탈로그에 저장된다. 따라서 시스템 카탈로그 클래스에 대해 대상 식별자를 검색하려면 소문자를 사용해야 한다. 반면, DB 사용자 이름은 대문자로 변경되어 _db_user 시스템 카탈로그 테이블에 저장된다.
