...
| 구분      | 타입           | 입력                    | 저장                              | 출력                                       | 설명                                                            |
+===========+================+=========================+===================================+============================================+=================================================================+
| DATE      | DATE           | 타임 존 없이            | 입력 값                           | 절대 값(입력 값과 동일)                    | 날짜                                                            |
+-----------+----------------+-------------------------+-----------------------------------+--------------------------------------------+-----------------------------------------------------------------+
| DATETIME  | DATETIME       | 타임 존 없이            | 입력 값                           | 절대 값(입력 값과 동일)                    | 밀리초를 포함한 날짜/시간                                       |
|           +----------------+-------------------------+-----------------------------------+--------------------------------------------+-----------------------------------------------------------------+
|           | DATETIMETZ     | 타임 존 포함하여        | UTC + 타임존(지역 또는 오프셋)    | 절대 값(입력한 타임 존 유지)               | 날짜/시간 + 타임존 정보                                         |
|           +----------------+-------------------------+-----------------------------------+--------------------------------------------+-----------------------------------------------------------------+
|           | DATETIMELTZ    | 타임 존 포함하여        | UTC                               | 상대 값(세션 타임 존에 따라 변환됨)        | 세션 타임존에서의 날짜/시간                                     |
+-----------+----------------+-------------------------+-----------------------------------+--------------------------------------------+-----------------------------------------------------------------+
| TIME      | TIME           | 타임 존 없이            | 입력 값                           | 절대 값(입력 값과 동일)                    | 시간                                                            |
|           +----------------+-------------------------+-----------------------------------+--------------------------------------------+-----------------------------------------------------------------+
|           | TIMETZ         | 타임 존 포함하여        | UTC + 타임존(지역 또는 오프셋)    | 절대 값(입력한 타임 존 유지)               | 시간 + 타임존 정보. 타임존 영역이 지명으로 표기되면             |
|           |                |                         |                                   |                                            | 날짜를 CURRENT_DATE()와 동일한 것으로 간주함                    |
|           +----------------+-------------------------+-----------------------------------+--------------------------------------------+-----------------------------------------------------------------+
|           | TIMELTZ        | 타임 존 포함하여        | UTC                               | 상대 값(세션 타임 존에 따라 변환됨)        | 세션 타임존에서의 시간. 타임존 영역이 지명으로 표기되면         |
|           |                |                         |                                   |                                            | 날짜를 CURRENT_DATE()와 동일한 것으로 간주함                    |
+-----------+----------------+-------------------------+-----------------------------------+--------------------------------------------+-----------------------------------------------------------------+
...
.. note:: **시간 타입의 타임존을 명시할 때 제약 사항**
    ...
    cubrid.conf의 timezone 파라미터의 값이 지역 이름으로 명시된 경우 TIMELTZ 타입에서 오프셋 타임존을 사용할 수 없다.
...
다음은 세션 타임존의 변경에 따라 TIME, TIMETZ와 TIMELTZ의 출력 값이 다르게 나타나는 예이다.
