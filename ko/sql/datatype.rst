...
**ENUM** 타입 칼럼으로 저장된 값을 **CHAR** 값으로 정렬하려면 열거형 값을 **CHAR** 타입으로 변환(CAST)한다.
...
**ALTER ... CHANGE** 문을 사용하여 **ENUM** 타입 값의 집합을 수정하려면 **alter_table_change_type_strict** 파라미터의 값이 반드시 yes여야 한다. 이 경우 시스템은 새로운 타입으로 변환한 열거형 값(문자열 리터럴)을 사용한다. 값이 새로운 **ENUM** 타입 값의 집합을 벗어나면 자동으로 **ENUM** 타입의 첫번째 값으로 매핑된다. 
