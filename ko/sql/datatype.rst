...
*   시스템 파라미터 **intl_date_lang**\ 을 설정하면 :func:`TO_DATE`, :func:`TO_DATETIME`, :func:`TO_TIMESTAMP` 함수의 입력 문자열 형식이 해당 로캘의 날짜 형식을 따른다. 자세한 내용은 :ref:`stmt-type-parameters`\ 과 각 함수의 설명을 참고한다.
...
:func:`CAST` 연산자를 사용하여 문자열을 날짜/시간 타입으로 변환할 때에는 문자열을 다음과 같은 형식으로 작성하는 것을 권장한다. 참고로, :func:`CAST` 연산자에서 사용하는 날짜/시간 문자열 형식은 로캘(**CUBRID_CHARSET** 환경 변수의 값으로 지정)의 영향을 받지 않는다.
...
**ENUM** 타입은 열거형 문자열 상수들의 순서 집합으로 구성되어 있는 타입이다. 열거형 칼럼을 생성하는 구문은 다음과 같다. ::
...
색인은 원소들이 열거형 타입에 정의된 순서에 따라 각 원소에 연관되어 있다. 예를 들어, *color* 칼럼은 다음 값 중 하나를 가질 수 있다(칼럼이 NULL 값을 허용한다고 가정).
...
**ENUM** 타입 값의 집합은 512개보다 많은 원소를 가질 수 없으며, 집합의 각 원소는 고유해야한다. 각 **ENUM** 타입 값에 대해 2바이트의 저장 공간이 할당되는데, 각 값의 색인만 저장하기 때문이다. 이러한 특징은 성능 향상에서 요구되는 저장 공간을 줄인다.

**ENUM** 타입을 가지고 작업할 때 열거형 값 또는 값의 색인 둘다 사용될 수 있다. 예를 들어, **ENUM** 타입 칼럼에 값을 삽입할 때 사용자는 **ENUM** 타입의 값 또는 색인을 사용할 수 있다. 
...
문맥 상 CHAR나 숫자 타입이 아닌 다른 타입으로 사용될 때, 열거형은 색인 또는 열거형 값을 사용하는 해당 타입으로 변환된다. 아래 테이블은 타입 변환 시 **ENUM** 타입의 어떤 부분이 사용되는지를 보여준다.
...
(<enum_칼럼> <연산자> <상수>) 형식의 연산자로 **=** 또는 **IN**\이 사용될 때, 시스템은 상수를 ENUM 타입으로 변환하려고 한다. 변환에 실패하면, 시스템은 오류를 반환하지 않지만 비교 결과를 FALSE로 반환할 것을 고려한다. 이와 같은 동작은 이러한 두 개의 연산자에서 생성되는 인덱스 스캔 질의 계획을 허용하기 위해서이다.

다른 모든 :doc:`비교 연산자<function/comparison_op>`\에 대해 **ENUM** 타입은 다른 피연산자의 타입으로 변환된다. 두 개의 **ENUM** 타입 사이에서 비교 연산이 수행되면 두 피연산자 모두 CHAR 타입으로 변환되며 CHAR 타입의 규칙을 따른다. **=**\과 **IN**\을 제외하고, **ENUM** 칼럼에서의 조건은 인덱스 스캔 질의 계획에서 사용될 수 없다.

이러한 규칙을 이해하기 위해 다음 테이블을 살펴볼 것이다.
...
다음 질의는 상수 'red'를 색인 번호 1을 가진 ENUM 값 'red'로 변환한다.
...
Values of the **ENUM** 타입 값은 열거되는 값에 의해 정렬되는 게 아니라, 값의 색인에 의해 정렬된다. **ENUM** 타입으로 칼럼이 정의할 때 열거되는 값의 순서도 정의한다. 
...
**ENUM** 타입 칼럼으로 저장된 값을 CHAR 값으로 정렬하려면 ENUM 값을 CHAR 타입으로 변환(CAST)한다.
...
**ENUM** 타입은 재사용 가능한 타입이 아니다. 여러 개의 칼럼이 같은 집합의 값을 요구한다면, 각 칼럼에 대해 **ENUM** 타입이 정의되어야 한다. **ENUM** 타입의 두 칼럼을 비교하는 경우 비록 두 **ENUM** 타입이 같은 집합 값을 정의했다 하더라도 두 칼럼이 CHAR 타입으로 변환된 것처럼 동작한다.

*ALTER ... CHANGE* 문을 사용하여 **ENUM** 타입 값의 집합을 수정하려면 **alter_table_change_type_strict** 파라미터의 값이 반드시 yes여야 한다. 이 경우 시스템은 새로운 타입으로 변환한 ENUM 값(문자열 리터럴)을 사용한다. 값이 새로운 **ENUM** 타입 값의 집합을 벗어나면 자동으로 **ENUM** 타입의 첫번째 값으로 매핑된다. 
...
다음 문장은 **ENUM** 타입이 'yellow' 값을 가지도록 한다.
...
다음 문장은 모든 튜플의 'green' 값이 'red' 값으로 바뀌는데, 'green' 값이 새로운 **ENUM** 타입으로 변환될 수 없기 때문이다.
