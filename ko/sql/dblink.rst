...
CUBRID DBLink는 FROM절에 DBLINK키워드를 이용하여 연결될 서버를 명시하고, 원격 서버에서 실행될 쿼리를 명시하는 형태로 사용할 수 있으며, 테이블 확장명 형태로도 사용이 가능하다. INSERT/UPDATE/DELETE/MERGE와 같은 DML 질의는 반드리 테이블 확장명으로만 사용할 수 있다.
...
*   테이블 확장명에는 일반 테이블, 시노님, 뷰 외에는 사용할 수 없다.  즉, 시리얼, 내장함수, 저장함수 등은 사용할 수 없다. 예를 들어, sp_func이 server1 원격서버에서 만들어진 저장함수라고하면 sp_func@server1처럼 사용할 수 없다.
*   SELECT 질의에 적용된 함수는 (SYSDATE 포함)내장 함수, 저장 함수를 막론하고 원격 DBMS의 종류와 상관 없이 로컬에서 동작한다.
*   INSERT/UPDATE/DELETE/MERGE DML 질의에서 사용된 함수는 (SYSDATE 포함)내장 함수, 저장 함수를 막론하고 원격 DBMS의 종류와 상관 없이 원격지에서 동작한다.
*   SELECT 질의에서 원격 함수를 사용해야 한다면, DBLINK절을 사용해서 사용자 질의에 해당 함수를 지정해야하고, 반대로 DML질의에서 CUBRID에 정의되어 있는 함수가 외부 DBMS에서는 없거나 동일한 함수라도 사용법이 다를 수 있으니 주의해야 한다.
