...
CUBRID DBLink는 FROM절에 DBLINK키워드를 이용하여 연결될 서버를 명시하고, 원격 서버에서 실행될 질의를 명시하는 형태로 사용할 수 있으며, 테이블 확장명 형태로도 사용이 가능하다. INSERT/UPDATE/DELETE/MERGE와 같은 DML 질의는 반드시 테이블 확장명으로만 사용할 수 있다.
