...
*   <*logical_expression*>: 불리언(논리) 값으로 평가되는 표현식이다. 비교 연산(예: a = 1)이나 논리 연산(예: (a = 1) AND (b = 2))의 결과가 해당한다.

다음은 CUBRID에서 지원되는 비교 연산자의 설명 및 반환 값을 나타낸 표이다.
...
|                         | 하나 이상의 피연산자가 **NULL** 이면 **NULL** 을 반환한다.                                  |                    |                |
+-------------------------+---------------------------------------------------------------------------------------------+--------------------+----------------+
| **IS**                  | 왼쪽 피연산자가 오른쪽 진리값과 같은지 비교한다.                                            | (1=1) IS FALSE     | 0              |
| *truth_value*           | 진리값은 **TRUE**, **FALSE**, **UNKNOWN** 이 될 수 있다.                                    |                    |                |
+-------------------------+---------------------------------------------------------------------------------------------+--------------------+----------------+
| **IS NOT**              | 왼쪽 피연산자가 오른쪽 진리값과 다른지 비교한다.                                            | (1=1) IS NOT FALSE | 1              |
| *truth_value*           | 진리값은 **TRUE**, **FALSE**, **UNKNOWN** 이 될 수 있다.                                    |                    |                |
+-------------------------+---------------------------------------------------------------------------------------------+--------------------+----------------+
