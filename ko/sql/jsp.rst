...
CUBRID의 Java SP에서는 JNI 기능을 사용할 수 있도록 제공하고 있지만 네이티브 코드의 문제가 저장 루틴 서버 (cub_javasp) 프로세스 및 그 동작에 예상하지 못한 영향을 줄 수 있으므로 주의해서 사용해야 한다.
...
.. warning::
    ...
    * 이미 로드된 자바 클래스 파일을 loadjava로 다시 덮어쓰는 경우
       * 클래스 파일이 새로운 클래스 로더를 통해 다시 로드되므로 위와 동일한 문제가 발생한다
       * javasp 유틸리티를 재시작한다
