...
재귀적 CTE는 무한 루프에 빠질수 있다. 이런 경우를 피하기 위해서 시스템 파라미터 **cte_max_recursions** 를 원하는 임계치로 설정하라. 이 파라미터의 기본값은 2000 재귀 반복이며, 최대값은 1000,000 최소값은 2이다.
...
.. warning::
    ...
    *   CTE 부질의의 복잡도에 따라서, 부질의의 결과는 매우 크게 증가할 수 있으며 결과적으로 많은 데이터를 생산한다. 또한 **cte_max_recursions** 의 기본값만으로도 디스크 공간 부족을 피하기에 충분하지 않을 수 있다.


재귀 CTE의 실행 알고리즘은 다음과 같이 요약 될 수 있다:
 * CTE의 비 재귀 부분을 실행하고 결과를 최종 결과 세트에 추가
 * 비 재귀 부분에서 얻은 결과 집합을 사용하여 재귀 부분을 실행, 최종 결과 집합에 결과를 추가하고 결과 집합 내에서 현재 반복의 시작과 끝을 기억
 * 이전 반복의 결과 세트를 사용하여 비 재귀 부분의 실행을 반복하고 결과를 최종 결과 세트에 추가
 * 재귀 반복이 결과를 생성하지 않으면 중지
 * 구성된 최대 반복 횟수에 도달하면 중지

재귀 CTE는 **FROM** 절에서 직접 참조해야하며 부질의에서 참조하면 오류가 발생된다.
