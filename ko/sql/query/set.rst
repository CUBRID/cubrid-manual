...
대부분의 SQL 문에서 사용자 정의 변수를 사용할 수 있지만, SELECT 문 내에서 변수의 지정과 참조 순서는 보장되지 않는다.
예를 들어, SELECT 목록에 지정된 사용자 정의 변수를 **HAVING**, **GROUP BY**, **ORDER BY** 절에서 참조하면 예상한 값이 반환되지 않을 수 있다.
또한, 사용자 정의 변수는 컬럼 이름이나 테이블 이름과 같은 식별자(identifier) 로는 사용할 수 없다.
