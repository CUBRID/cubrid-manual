...
*   드라이버 연결 종료
*   자동 커밋 ON 모드에서 질의 실행 오류 또는 자동 커밋 OFF 모드에서 사용자의 명시적인 롤백 호출로 인해 트랜잭션이 롤백되는 경우
