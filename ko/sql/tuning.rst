...
재 실행되는 상관 부질의의 참조한 본 질의의 컬럼 값이 동일한 경우 부질의 수행 대신 캐시된 결과를 사용하며,
캐시된 값을 찾을 수 없는 경우 부질의 수행 후 참조한 컬럼 값과 결과를 서브 쿼리 캐시에 저장한다. 
같은 컬럼 값이 캐시에서 발견되면 캐시된 영역에서 결과를 가져온다. 

SQL 트레이스 정보를 요청과 함께 질의 수행시 서브 쿼리 캐시 최적화가 적용된 부질의의 하위 정보로 서브 쿼리 캐시에 대한 트레이스 정보가 출력된다.
...
첫 번째 예제는 상관 부질의에 서브 쿼리 캐쉬를 사용하여 질의 수행 시간을 출력하였고, 두 번째 예제는 상관 부질의에  **NO_SUBQUERY_CACHE** 힌트를 사용하여 서브 쿼리 캐시를 비활성화한 후 질의 수행 시간을 출력하였다.
두 예제의 응답시간을 비교해보면 서브 쿼리 캐시를 사용한 경우 향상되는 것을 확인할 수 있다. 
...
다음 예시는 상관 부질의가 상관 부질의를 포함한 경우의 서브 쿼리 캐시가 비활성화되어 수행되는 예시이다. 
