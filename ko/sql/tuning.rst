...
이 외에도 inner 테이블에 컬렉션 타입(**SET**, **MULTISET**, **LIST**) 컬럼이 있는 경우나 시스템 클래스를 조회하는 경우와 같이 캐시된 결과의 재사용이 안전하지 않은 일부 경우에는 적용되지 않는다.
...
이 경우 **enabled**\ 는 하나의 워커에서라도 캐시가 해제되었으면 false로 출력된다.
