...
**FUNC**: 해당 질의에서 저장 함수를 호출하는 경우에 출력된다. fetch와 ioread는 저장 함수 내 질의 수행 시에 발생한 회수는 포함하지 않는다.

*   time: 해당 질의에서 저장 함수 전체 수행 시간(ms)
*   fetch: 해당 질의에 저장 함수 호출 중 페이지를 fetch한 회수.
*   ioread: 해당 질의에 저장 함수 호출 중 발생한 전체 I/O 읽기 회수. 데이터를 읽을 때 물리적으로 디스크에 접근한 회수
*   calls: 해당 질의에 저장 함수 호출 횟수
