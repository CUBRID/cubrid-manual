...
*   **PARALLEL** ( *degree* ): 병렬 쿼리 실행(병렬 힙 스캔, 병렬 비상관부질의 실행, 병렬 해시 조인, 병렬 정렬)을 활성화하고 병렬 처리 정도를 지정하는 힌트이다. *degree* 는 2 이상의 정수 값이어야 하며, 병렬로 처리할 워커 스레드의 수를 의미한다. 자세한 내용은 :ref:`parallel-query`\를 참고한다.
...
*   **NO_PARALLEL_SUBQUERY**: 비상관 부질의(uncorrelated subquery)의 병렬 실행을 사용하지 않도록 하는 힌트이다. 자세한 내용은 :ref:`parallel-query`\를 참고한다.
