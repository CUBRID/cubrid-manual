...
인덱스 스캔은 논리프 노드, 리프 노드 그리고 힙 영역의 탐색으로 진행된다. 인덱스 스캔의 시작은 루트노드를 포함한 논리프 노드를 탐색하여 리프노드의 시작점을 찾는 것이다. 그리고 리프노드를 탐색하며 만족하는 키의 OID정보를 조회하고 필요하다면 OID 정보로 힙영역을 스캔한다. 아래의 예제는 어떻게 인덱스 스캔이 수행되는지 보여준다.
