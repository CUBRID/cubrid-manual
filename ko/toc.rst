...
*   :doc:`ha`: High Availability 기능을 CUBRID에 적용한 것이다. 여러 서버에서 데이터베이스를 항상 동기화된 상태로 유지고, 서비스 중인 서버에 장애가 발생하면 자동으로 다른 서버가 서비스를 수행한다.

*   :doc:`shard`: sharding은 물리적으로 다른 데이터베이스에 데이터를 수평분할 방식으로 분산 저장하고 조회하는 방법을 말한다. CUBRID SHARD는 하나의 데이터베이스 인스턴스에 넣을 수 없는 큰 데이터와 처리량을 분산하여 처리하기 위해 사용된다.
