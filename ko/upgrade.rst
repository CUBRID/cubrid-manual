...
보다 자세한 설명은 릴리스 노트의 :ref:`changed-config93`\ 을 참고한다.

.. _up-from-41:

CUBRID 2008 R4.1/R4.3에서 CUBRID 9.3으로 업그레이드하기
-------------------------------------------------------

CUBRID 2008 R4.1/R4.3 버전 사용자는 CUBRID 9.3 버전을 별도의 디렉터리에 설치한 후 기존의 환경 설정 파일에서 파라미터들의 값을 변경해야 한다.
...
아래는 cubrid unloaddb/loaddb 유틸리티와 http://ftp.cubrid.org/CUBRID_Engine/9.3.0/Linux/\ 에서 별도 배포되는 check_reserved.sql 예약어 검출 스크립트를 이용하여 마이그레이션을 수행하는 방법이다. (cubrid unloaddb/loaddb는 매뉴얼의 :ref:`unload-load` 참고)
...
보다 자세한 설명은 :ref:`changed-config93`\ 을 참고한다.
