...
.. note::
    ...
    11.2 버전부터 논리식이 아닌 값(정수·컬럼 등)을 피연산자로 사용하면 "operand must be logical expression." 오류가 발생한다. 예를 들어 ``WHERE 1`` 또는 ``WHERE column_name`` 과 같이 비논리 값을 직접 조건절에 사용하던 구문은 ``WHERE column_name <> 0`` 처럼 명시적인 비교 연산으로 작성해야 한다.
