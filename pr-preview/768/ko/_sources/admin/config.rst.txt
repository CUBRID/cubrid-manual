...
**parallelism**
    ...
    **PARALLEL** ( *degree* ) 힌트를 사용하여 질의별로 병렬 처리 수준을 재정의할 수 있다. 자세한 내용은 :ref:`parallel-query`\ 를 참고한다.
    ...
    이 파라미터는 cubrid.conf에 설정한 후 서버를 재시작해야 적용되며, **SET SYSTEM PARAMETERS** 문으로는 변경할 수 없다. 서버 기동 시 이 파라미터 값이 :ref:`max_parallel_workers <max_parallel_workers>` 값보다 크면 그 값으로 낮추어 적용된다.
