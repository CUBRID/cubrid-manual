...
*   :doc:`install_upgrade`: CUBRID 설치와 실행, 환경변수 설정, 포트설정, 업그레이드, HA 환경에서 DB 마이그레이션, CUBRID 제거에 대해서 설명한다.
...
*   :doc:`pl/index`: 저장 프로시저와 저장 함수를 사용할 수 있도록 CUBRID 에서 제공하는 절차적 언어 확장 기능에 대해서 설명한다.  저장 프로시저와 저장 함수는 SQL로 구현하기 어려운 복잡한 비즈니스 로직을 처리하고, 데이터 조작을 간편하게 수행할 수 있도록 도와준다.
...
*   :doc:`ha`: High Availability 기능을 CUBRID에 적용한 것이다. 여러 서버에서 데이터베이스를 항상 동기화된 상태로 유지하고, 서비스 중인 서버에 장애가 발생하면 자동으로 다른 서버가 서비스를 수행한다.

*   :doc:`shard`: sharding은 물리적으로 다른 데이터베이스에 데이터를 수평 분할 방식으로 분산 저장하고 조회하는 방법을 말한다. CUBRID SHARD는 하나의 데이터베이스 인스턴스에 넣을 수 없는 큰 데이터와 처리량을 분산하여 처리하기 위해 사용된다.
