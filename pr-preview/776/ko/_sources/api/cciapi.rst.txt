...
.. c:function:: int cci_fetch_sensitive(int req_handle, T_CCI_ERROR *err_buf)
    ...
    sensitive column이란 **SELECT** 목록 항목 중 결과 재요청 시 업데이트된 값을 제공할 수 있는 항목을 말한다. 주로 어떠한 연산 없이, 예를 들면 집계 연산과 같은 과정이 없이 칼럼을 **SELECT** 목록의 항목으로 그대로 쓰는 경우 그 칼럼을 sensitive column이라고 말할 수 있다.
...
.. c:macro:: CCI_GET_RESULT_INFO_ATTR_NAME(res_info, index)
    ...
    prepare된 **SELECT** 목록에서 *index* 번째 칼럼의 실제 속성 이름을 가져오는 매크로이다. 속성 이름이 없는 경우(상수값, 함수 등)는 빈 문자열 (empty string)을 반환한다. 지정된 인자 *res_info* 가 **NULL** 인지, *index* 가 유효한지에 대한 검사는 하지 않는다. 반환된 메모리 포인터는 사용자가 **free**\ ()를 통해 제거할 수 없다.
...
.. c:macro:: CCI_GET_RESULT_INFO_CLASS_NAME(res_info, index)
    ...
    prepare된 **SELECT** 목록에서 *index* 번째 칼럼의 클래스 이름을 가져오는 매크로이다. 지정된 인자 *res_info* 가 **NULL** 인지, *index* 가 유효한지에 대한 검사는 하지 않는다. 반환된 메모리 포인터는 사용자가 **free**\ ()를 통해 제거할 수 없다. 반환된 값은 **NULL**\ 을 가질 수 있다.
...
.. c:macro:: CCI_GET_RESULT_INFO_IS_NON_NULL(res_info, index)
    ...
    prepare된 **SELECT** 목록에서 *index* 번째 칼럼이 nullable인지에 대한 값을 가져오는 매크로이다. 지정된 인자 *res_info* 가 **NULL** 인지, *index* 가 유효한지에 대한 검사는 하지 않는다. 
   
    **SELECT** 목록의 칼럼이 테이블의 칼럼이 아닌 표현식인 경우 NON_NULL 여부를 알 수 없으므로 CCI_GET_RESULT_INFO_IS_NON_NULL 매크로는 일관되게 0을 반환한다.
...
.. c:macro:: CCI_GET_RESULT_INFO_NAME(res_info, index)
    ...
    prepare된 **SELECT** 목록에서 *index* 번째 칼럼의 이름을 가져오는 매크로이다. 지정된 인자 *res_info* 가 **NULL** 인지, *index* 가 유효한지에 대한 검사는 하지 않는다. 반환된 메모리 포인터는 사용자가 **free**\ ()를 통해 제거할 수 없다.
...
.. c:macro:: CCI_GET_RESULT_INFO_PRECISION(res_info, index)
    ...
    prepare된 **SELECT** 목록에서 *index* 번째 칼럼의 precision을 가져오는 매크로이다. 지정된 인자 *res_info* 가 **NULL** 인지, *index* 가 유효한지에 대한 검사는 하지 않는다.
...
.. c:macro:: CCI_GET_RESULT_INFO_SCALE(res_info, index)
    ...
    prepare된 **SELECT** 목록에서 *index* 번째 칼럼의 scale을 가져오는 매크로이다. 지정된 인자 *res_info* 가 **NULL** 인지, *index* 가 유효한지에 대한 검사는 하지 않는다.
...
.. c:macro:: CCI_GET_RESULT_INFO_TYPE(res_info, index)
    ...
    prepare된 **SELECT** 목록에서 *index* 번째 칼럼의 타입을 가져오는 매크로이다. 지정된 인자 *res_info* 가 **NULL** 인지, *index* 가 유효한지에 대한 검사는 하지 않는다.  
