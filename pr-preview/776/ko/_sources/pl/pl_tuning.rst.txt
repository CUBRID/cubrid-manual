...
* **통신 오버헤드**: CUBRID 데이터베이스 서버와 CUBRID PL 서버 간의 통신 오버헤드를 줄이기 위해서는 최소한의 개수의 레코드를 반환하도록 질의를 작성해야 한다.
...
CUBRID에서 기본적으로 제공하는 내장 함수는 (:doc:`/sql/function/index`) CUBRID의 질의 실행 동작에 맞춰 최적화되어 효율적으로 동작한다.
...
위 예시에서 pl_csql_not_deterministic 함수는 **NOT DETERMINISTIC**\이므로 상관 부질의에서 질의 캐시를 사용하지 않는다.
반면, pl_csql_deterministic 함수는 **DETERMINISTIC** 키워드가 지정되어 있으므로 상관 부질의에서 질의 캐시를 사용하여 성능을 최적화할 수 있다.
