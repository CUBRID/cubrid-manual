...
*   **리스트(List)**: 임시 결과 리스트 파일 — **섹터** 단위로 워커들에게 사전 분할
...
*   임시 리스트가 메모리 버퍼에만 존재하고 디스크 임시 파일로 저장되지 않은 경우 (분할할 섹터가 없어 작은 리스트는 자동으로 단일 스레드로 실행)
...
병렬 해시 조인은 두 입력 리스트 중 **큰 쪽의 페이지 수**\ 를 기준으로 스캔과 같은 형태의 규칙(활성화 조건 2,048페이지, 페이지 수가 2배가 될 때마다 병렬 처리 수준 1 증가, :ref:`parallelism <parallelism>` 상한)에 따라 병렬 처리 수준이 계산된다.
...
병렬 정렬은 정렬 입력 리스트의 페이지 수를 기준으로 스캔과 같은 형태의 규칙(활성화 조건 2,048페이지, 페이지 수가 2배가 될 때마다 병렬 처리 수준 1 증가, :ref:`parallelism <parallelism>` 상한)에 따라 병렬 처리 수준이 계산된다.
