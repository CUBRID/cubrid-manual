...
분할을 생성할 때, 사용자가 제공한 영역을 가장 작은 값부터 가장 큰 값까지 정렬하고 정렬된 목록에서 겹치지 않는 간격을 생성한다. 위 예에서 생성된 영역의 간격은 [-inf, 2000)와 [2000, 2008)이다. 분할에 대한 무제한의 최대값을 지정하고 싶으면 **MAXVALUE** 식별자를 사용한다.
...
분할 주석을 확인하는 방법은 :ref:`show-partition-comment`\를 참고한다.
...
리스트 분할은 사용자가 지정한 분할 키 값의 목록에 따라 테이블을 분할하는 기법이다. 분할을 위한 값의 목록은 겹치는 값이 없어야 한다. 이 분할 기법은 사원 테이블의 부서 ID, 사용자 테이블의 국가 코드와 같은 경우처럼 테이블 데이터가 의미 있는 값의 목록으로 나누어질 때 유용하다. 해시 분할과 마찬가지로, 리스트 분할에 대한 :ref:`partition-pruning` 최적화는 동등 조건(**=**\과 :ref:`IN <in-expr>` 조건)에만 적용된다. 
...
리스트 분할 테이블에 투플을 삽입할 때 분할 키 값은 분할에 정의된 목록 값 중 하나에 속해야 한다. 리스트 분할의 경우 분할 키 값이 **NULL**\일 때 자동으로 특정 분할을 할당하지 않고 오류가 발생된다. **NULL** 값을 저장하려면 다음의 예와 같이 **NULL**\을 포함하는 분할을 생성해야 한다.
...
분할 주석은 영역 분할과 리스트 분할에 대해서만 지정할 수 있으며, 해시 분할에서는 지정할 수 없다. 분할 주석은 다음 구문을 실행하여 확인할 수 있다.
...
함수 표현식을 사용하지 않고 원본 칼럼만 사용하는 경우에 대해, 분할 프루닝의 적용 여부를 확인한다.
...
분할 키로 정의한 함수 표현식이 원본 칼럼과 동일한 정렬 순서를 보장하는 경우에는,
함수 표현식 대신 원본 칼럼을 사용하더라도 분할 프루닝이 적용될 수 있다.
...
질의 실행 후 프로파일링 결과를 확인하면,
함수 표현식 분할 키 조건을 직접 사용하지 않았음에도 **YEAR** 함수가 원본 칼럼과 동일한 정렬 순서를 보장하므로 분할 프루닝이 적용되었음을 확인할 수 있다.
...
따라서 조인 실행 시점에 칼럼 값이 확정되는 조인 조건으로는 질의 실행 준비 단계에서 값을 미리 알 수 없으므로, 분할 프루닝이 적용되지 않는다.
