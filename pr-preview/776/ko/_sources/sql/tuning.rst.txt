...
통계 정보 갱신 시작과 종료 시 서버 오류 로그에 NOTIFICATION 메시지를 출력하며, 이를 통해 통계 정보 갱신에 걸리는 시간을 확인할 수 있다.
...
*   B+tree statistics: 인덱스 통계정보이다.
    ...
    *   *B+tree Cardinality*: 인덱스 key 값의 누적된 중복이 제거된 값의 개수이다. 위 예제에서 **(5,5)**\는 인덱스 칼럼 **(code,name)**\과 매칭된다. 첫 번째 **5**\는 **code** 칼럼의 중복이 제거된 값의 개수이고, 두 번째 **5**\는 **code,name** 두 칼럼의 값의 중복이 제거된 개수이다. *Number of Distinct Values*\와 달리 앞 칼럼과 함께 누적된 중복이 제거된 값의 개수이므로 5개이다.
...
*   *opt-level* : 최적화 수준을 지정하는 값으로 다음과 같은 의미를 갖으며, 이 외에 값을 설정할 경우 오류를 발생한다.
...
*   cost: CPU, IO 등 주로 자원의 사용과 관련하여 비용을 내부적으로 산정한다. 
...
*   noscan: aggregate 연산 시 스캔 없이 인덱스 헤더의 통계 정보를 이용하는 작업 (aggregate: count, min, max)
    ...
    *   agl: aggregate lookup을 의미하며, aggreate 연산을 위해 사용된 인덱스 목록
...
*   **PARALLEL** ( *degree* ): 병렬 질의 실행(병렬 스캔(힙/목록/인덱스), 병렬 부질의 실행, 병렬 해시 조인, 병렬 정렬)을 활성화하고 병렬 처리 정도를 지정하는 힌트이다. *degree* 는 0 이상의 정수 값이어야 하며, 병렬로 처리할 워커 스레드의 수를 의미한다. 0이나 1로 지정할 경우 병렬 처리 기능이 비활성화되며, 시스템의 CPU 코어 수를 초과하는 값은 코어 수로 낮추어 적용된다. 힌트를 지정해도 대상의 페이지 수가 활성화 조건에 미달하면 병렬 실행이 적용되지 않는다. 자세한 내용은 :ref:`parallel-query`\를 참고한다.
...
*   **NO_PARALLEL_SCAN**: 해당 질의 블록의 모든 병렬 스캔(힙/목록/인덱스)을 사용하지 않도록 하는 힌트이다. **PARALLEL** 힌트와 같이 사용하는 경우에는 **NO_PARALLEL_SCAN** 이 우선 적용된다. 자세한 내용은 :ref:`parallel-query`\를 참고한다.
...
**USING INDEX** 절에 인덱스 이름의 목록이 지정되면 질의 최적화기는 지정된 인덱스만을 대상으로 질의 실행 비용을 계산하고, 지정된 인덱스의 인덱스 스캔 비용과 순차 스캔 비용을 비교하여 최적의 실행 계획을 만든다(CUBRID는 실행 계획을 선택할 때 비용 기반의 질의 최적화를 수행한다).
...
질의 수행 시 **SELECT** 목록, **WHERE**, **HAVING**, **GROUP BY**, **ORDER BY** 절에 있는 모든 칼럼의 데이터를 포함하는 인덱스를 커버링 인덱스(covering index)라고 한다.
...
*   조인 질의의 경우 인덱스가 **SELECT** 목록에 있는 테이블의 칼럼을 포함하면, 이 인덱스를 사용한다.
...
다음의 예는 커버링 인덱스 사용시 **SELECT** 목록에 **COUNT(*)** 만 있는 경우 불필요한 스캔을 줄여주는 최적화를 보여준다.
...
위의 테이블과 인덱스를 생성했을 때 다음의 예는 k1, k2 칼럼으로 **GROUP BY**\를 수행하며 k3로 집계 함수를 수행하므로 tab(k1, k2, k3)로 구성된 인덱스가 사용되고 별도의 정렬 과정이 필요 없다. 또한 **SELECT** 목록에 있는 k1, k2, k3 칼럼이 모두 tab(k1, k2, k3)로 구성된 인덱스 내에 존재하므로 커버링 인덱스가 적용된다.
...
다음의 예는 *k1*, *k2* 칼럼으로 **GROUP BY**\를 수행하므로 *tab*(*k1*, *k2*, *k3*)로 구성된 인덱스가 사용되고 별도의 정렬 과정이 필요 없다. 하지만 **SELECT** 목록에 있는 v 칼럼은 *tab*(*k1*, *k2*, *k3*)로 구성된 인덱스 내에 존재하지 않으므로 커버링 인덱스가 적용되지 않는다.
...
**INDEX_SS** 힌트에는 ISS의 적용을 고려할 테이블 목록을 입력할 수 있으며,
테이블 목록이 생략되는 경우 모든 테이블에 대해 ISS의 적용이 고려된다.
...
1.  인덱스가 **SELECT** 목록의 모든 부분을 커버하는 경우. 즉, 커버링 인덱스가 적용되는 경우
...
부모-자식 관계의 **INNER JOIN**\에서 조인 조건 외에 부모 테이블에 대한 참조가 없는 경우에는 부모 테이블과의 조인을 제거해도 질의 결과에 영향을 주지 않는다.
이러한 경우에는 부모 테이블과의 조인을 제거하여 조인 연산을 줄이고, 질의 성능을 향상시킨다.

**INNER JOIN**\을 제거하기 위해서는 다음 조건을 만족해야 한다:

    #. 부모-자식 관계의 **INNER JOIN**\을 한다.
    #. 기본키(**PK**)와 외래키(**FK**)의 모든 칼럼은 조인 조건에 사용되어야 한다.
    #. 기본키(**PK**) 칼럼은 해당 기본키(**PK**) 칼럼을 참조하는 외래키(**FK**) 칼럼과 동일한 조인 조건에 사용되어야 한다.
    #. 모든 조인 조건은 동등(=) 비교를 사용해야 한다.
    #. 조인 조건 외에 부모 테이블에 대한 참조가 없어야 한다.
    #. 상속 관계가 있는 테이블을 조회하는 경우에는 **ALL** 키워드를 사용하지 않아야 한다.

부모 테이블과의 조인을 제거할 때 자식 테이블의 외래키(**FK**) 칼럼에 **NOT NULL** 제약 조건이 없는 경우에는 해당 칼럼에 대한 **IS NOT NULL** 조건이 추가된다.
...
*parent_tbl* 테이블과의 조인이 제거되었고, *child_tbl* 테이블의 *parent_sub_id* 칼럼에 대한 **IS NOT NULL** 조건이 추가되었다.
...
다음은 *parent_tbl* 테이블의 *id* 칼럼과 *child_tbl* 테이블의 *parent_id* 칼럼만 조인 조건으로 사용된 예제이다.
...
*parent_tbl* 테이블의 *sub_id* 칼럼과 *child_tbl* 테이블의 *parent_sub_id* 칼럼이 조인 조건으로 사용되지 않았기 때문에 *parent_tbl* 테이블과의 조인이 제거되지 않았다.
...
기본키(**PK**) 칼럼은 해당 기본키(**PK**)를 참조하는 외래키(**FK**) 칼럼과 동일한 조인 조건에 사용되어야 한다.
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

다음은 조인 조건에서 *id* 칼럼을 *parent_sub_id* 칼럼과 비교하고, *sub_id* 칼럼을 *parent_id* 칼럼과 비교하는 예제이다.
...
*id* 칼럼이 *parent_id* 칼럼과 비교되지 않았고, *sub_id* 칼럼이 *parent_sub_id* 칼럼과 비교되지 않았기 때문에 *parent* 테이블과의 조인이 제거되지 않았다.
...
조인 조건에서 *parent_tbl* 테이블의 *sub_id* 칼럼과 *child_tbl* 테이블의 *parent_sub_id* 칼럼을 비교할 때 동등(=) 비교를 사용하지 않았기 때문에 *parent* 테이블과의 조인이 제거되지 않았다.
...
다음은 *parent_tbl* 테이블의 *name* 칼럼을 조회하는 예제이다.
...
*parent_tbl* 테이블의 *name* 칼럼의 값을 조회하고 있기 때문에 *parent_tbl* 테이블과의 조인이 제거되지 않았다.
...
다음은 *parent* 테이블의 *filter* 칼럼에 대한 조건이 있는 예제이다.
...
*parent_tbl* 테이블의 *filter* 칼럼의 값이 5인 레코드를 조회하고 있기 때문에 *parent_tbl* 테이블과의 조인이 제거되지 않았다.
...
다음은 *parent_tbl* 테이블의 *name* 칼럼을 기준으로 결과를 정렬하는 예제이다.
...
*parent_tbl* 테이블의 *nmae* 칼럼을 기준으로 결과를 내림차순 정렬하고 있기 때문에 *parent_tbl* 테이블과의 조인이 제거되지 않았다.
...
**WHERE**\절에서 조인 조건 외에 부모 테이블에 대한 조건이 있더라도 조인 조건에 사용된 칼럼이 해당 조건에 사용된 경우에는 묵시적인 동등(=)에 의해 부모 테이블과의 조인을 제거할 수 있다.
해당 조건은 동등(=) 비교를 사용해야 하고, 상수와 비교되어야 한다.

다음은 *parent_tbl* 테이블의 *sub_id* 칼럼에 대한 동등(=) 비교 조건이 있는 예제이다.
...
*parent_tbl* 테이블의 *sub_id* 칼럼의 값이 60인 레코드를 조회하고 있지만 조인 조건에 의해 해당 조건은 *child_tbl* 테이블의 *parent_sub_id* 칼럼의 값이 60인 조건과 동등하기 때문에 *parent_tbl* 테이블과의 조인이 제거되었다.
...
N:1 관계의 **LEFT OUTER JOIN**\에서 조인 조건 외에 오른쪽 테이블에 대한 참조가 없는 경우에는 오른쪽 테이블과의 조인을 제거해도 질의 결과에 영향을 주지 않는다.
이러한 경우에는 오른쪽 테이블과의 조인을 제거하여 조인 연산을 줄이고, 질의 성능을 향상시킨다.

**LEFT OUTER JOIN** 제거를 하기 위해서는 다음 조건을 만족해야 한다:

    #. **LEFT OUTER JOIN**\을 한다.
    #. 오른쪽 테이블은 기본키(**PK**) 또는 **UNIQUE** 제약조건이 있어야 한다.
    #. 기본키(**PK**) 또는 **UNIQUE** 제약조건의 모든 칼럼은 조인 조건에 사용되어야 한다.
    #. 모든 조인 조건은 동등(=) 비교를 사용해야 한다.
...
다음은 *right_tbl* 테이블의 기본키(**PK**)를 제거하고 *id* 칼럼과 *sub_id* 칼럼으로 구성된 일반 인덱스를 생성해놓은 예제이다.
...
다음은 *right_tbl* 테이블의 기본키(**PK**)를 제거하고 *id* 칼럼과 *sub_id* 칼럼으로 구성된 **UNIQUE** 제약조건을 생성해놓은 예제이다.
...
기본키(**PK**) 또는 **UNIQUE** 제약조건의 모든 칼럼을 조인 조건에 사용해야 한다.
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

다음은 *left_tbl* 테이블의 *id* 칼럼과 *right_tbl* 테이블의 *id* 칼럼만 조인 조건으로 사용된 예제이다.
...
*left_tbl* 테이블의 *sub_id* 칼럼과 *right_tbl* 테이블의 *sub_id* 칼럼이 조인 조건으로 사용되지 않았기 때문에 *right_tbl* 테이블과의 조인이 제거되지 않았다.
...
조인 조건에서 *left_tbl* 테이블의 *sub_id* 칼럼과 *right_tbl* 테이블의 *sub_id* 칼럼을 비교할 때 동등(=) 비교를 사용하지 않았기 때문에 *right_tbl* 테이블과의 조인이 제거되지 않았다.
...
다음은 *right_tbl* 테이블의 *name* 칼럼을 조회하는 예제이다.
...
*right_tbl* 테이블의 *name* 칼럼의 값을 조회하고 있기 때문에 *right_tbl* 테이블과의 조인이 제거되지 않았다.
...
다음은 **LEFT OUTER JOIN**\을 하기 전에 *right_tbl* 테이블의 *filter* 칼럼에 대한 조건이 있는 예제이다.
...
*right_tbl* 테이블의 *filter* 칼럼의 값이 5인 레코드를 조회하고 있지만 **LEFT OUTER JOIN**\을 하기 전이기 때문에 *right_tbl* 테이블과의 조인이 제거되었다.
...
다음은 *right_tbl* 테이블의 *name* 칼럼으로 결과를 정렬하는 예제이다.
...
*right_tbl* 테이블의 *name* 칼럼을 기준으로 결과를 내림차순 정렬하고 있기 때문에 *right_tbl* 테이블과의 조인이 제거되지 않았다.
...
위 질의 중 인라인 뷰 내부에는 조건절이 없다. 만약 질의 변환이 수행되지 않았다면,
...
.. note::
    ...
    CUBRID 11.0 부터 **IN** 연산자의 인자에 여러개의 칼럼을 조회하는 부질의가 허용 된다.
...
**QUERY_CACHE** 힌트는 반복적으로 실행되는 질의의 성능을 향상시키는 데 사용할 수 있으며, 질의는 전용 메모리 영역에 캐시되고 그 결과도 별도의 디스크 공간에 캐시된다. 단, 힌트는 SELECT 질의에만 적용된다. 그러나 다음과 같은 경우에는 질의에 힌트를 적용 할 수 없으며 힌트는 의미가 없어진다.
...
힌트가 설정되고 새 SELECT 질의가 처리될 때 질의가 캐시에서 발견되면 질의 캐시를 사용한다. 동일한 데이터베이스에서 동일한 질의 텍스트와 동일한 바인딩 값을 사용하는 경우 질의는 동일한 것으로 간주된다. 캐시된 질의를 찾을 수 없는 경우 질의가 처리된 다음 결과와 함께 캐시가 생성된다. 질의가 캐시에서 발견되면 캐시된 영역에서 결과를 가져온다. CSQL에서는 아래 예제와 같이 COUNT 함수를 사용하여 질의를 반복적으로 실행할 때 개선된 성능을 쉽게 측정 할 수 있다. 첫번째 질의에 대한 결과는 캐시가 되어 있지 않아서 느리지만, 두 번째 질의의 결과는 캐시 된 영역에서 가져오므로 응답 시간이 이전 질의보다 훨씬 빠르다. ::
...
다음과 같이 CSQL에 세션 명령 *;info qcache* 를 입력하여 질의가 캐시되는지 여부를 확인할 수 있다. ::
...
위 UNION 질의에서 첫 번째 질의는 캐시 되지만, 두 번째 질의는 다른 테이블을 참조하기 때문에 캐시 되지 않는다.
...
:ref:`질의 프로파일링 <query-profiling>` 요청과 함께 질의 수행시 비상관 부질의(uncorrelated subquery) 캐시가 적용된 부질의의 하위 정보로 비상관 부질의 캐시에 대한 프로파일링 정보가 출력된다.

다음은 부질의 수행시 부질의 캐시 프로파일링 정보가 표시되는 예시이다.
...
비상관 부질의 캐시의 프로파일링 항목에 대한 설명은 다음과 같다.
...
부질의 캐시 최적화는 상관 부질의(correlated subquery)를 포함한 질의 성능을 향상시키는데 사용할 수 있으며, 부질의 결과는 부질의마다 독립적인 공간에 캐시된다. 
부질의 캐시 최적화는 기본적으로 적용되므로 질의 수행시 캐시를 사용하지 않으려면 **NO_SUBQUERY_CACHE** 힌트를 해당 부질의에 사용해야 한다.

부질의 캐시 최적화는 상관 부질의가 SELECT 절에 있는 경우 적용되며, 처리 절차는 다음과 같다. 
재 실행되는 상관 부질의의 검색 조건 칼럼 값이 동일한 경우 부질의 수행 대신 캐시된 결과를 사용한다.
만약, 캐시된 값을 찾을 수 없는 경우 부질의 수행 후 참조한 칼럼 값과 결과를 부질의 캐시에 저장한다. 

:ref:`질의 프로파일링 <query-profiling>` 요청과 함께 질의 수행시 부질의 캐시 최적화가 적용된 부질의의 하위 정보로 부질의 캐시에 대한 프로파일링 정보가 출력된다.

다음은 상관 부질의 수행시 부질의 캐시 프로파일링 정보가 표시되는 예시이다
...
부질의 캐시의 프로파일링 항목에 대한 설명은 다음과 같다.

* **hit** : 질의 실행 대신 캐시된 영역에서 결과를 가져온 횟수.
* **miss** : 질의를 실행한 후 결과를 캐시한 횟수.
* **size** : 부질의 캐시에 사용된 메모리 크기.
* **status** : 질의 종료 시 부질의 캐시의 활성화 여부.

**size**\가 설정해둔 값을 초과하는 경우에는 부질의 캐시가 질의 수행 도중 비활성화 되며 SQL 트레이스 정보에서 **status**\가 disabled로 출력된다. 
또한, **hit** 비율이 90% 미만일 경우, 부질의 캐시의 메모리 크기가 설정해둔 값을 초과하지 않더라도 캐시 miss가 높다고 판단해서 질의 수행 도중 캐시가 비활성화 될 수 있다. 
...
다음은 부질의 캐시의 사용 유무에 따른 성능 차이를 확인하기 위해 반복 수행되는 상관 부질의 결과를 count하는 예제 질의이다.
...
질의 #1는 부질의 캐시 최적화를 사용하여 질의를 수행했고, 질의 #2는  **NO_SUBQUERY_CACHE** 힌트를 사용하여 부질의 캐시를 비 활성화하여 질의를 수행했다. 
두 질의의 결과를 비교해보면 부질의 캐시 최적화를 사용한 것이 응답 수행이 개선된 것을 확인할 수 있다. 
...
부질의 캐시는 다음과 같은 상황에 동작하지 않는다:

* 상관 부질의가 상관 부질의를 포함한 경우. (단, 다른 상관 하위 질의가 포함되지 않은 최 하단의 상관 부질의는 부질의 캐시 최적화 작업이 수행됨)
* 부질의가 단일 행 부질의가 아닌 경우.
* 부질의가 CONNECT BY 절을 포함한 경우.
* 부질의에 OID 관련 기능이 포함된 경우.
* 부질의가 **NO_SUBQUERY_CACHE** 힌트를 포함한 경우.
* 새로운 결과를 저장할 때 설정한 부질의 캐시 크기(기본값: 2MB)를 초과하는 경우.
* random (), sys_guid ()와 같이 실행될 때마다 결과가 바뀌는 함수가 포함된 경우.

다음 예시는 상관 부질의가 상관 부질의를 포함한 경우, 최하단의 상관부질의만 부질의 캐시가 사용되고, 상단의 상관 부질의에는 부질의 캐시가 적용되지 않는 예시이다. 
...
다음은 상관 부질의에 random () 와 같이 실행될 때마다 결과가 바뀌는 함수가 포함된 경우 부질의 캐시가 비활성화되는 예시이다.
...
*   조인 조건에서 outer 테이블의 칼럼 값(조인 키)을 찾을 수 있다. 조인 조건이 없는 크로스 조인에는 적용되지 않는다.
...
*   inner 테이블에 컬렉션 타입(**SET**, **MULTISET**, **LIST**) 칼럼이 있는 경우
...
이때 질의는 실패하지 않으며, 오류나 경고도 발생하지 않는다.
...
다음은 예제 질의를 수행하기 위한 데이터를 준비하는 질의이다. *orders* 테이블의 *customer_id* 칼럼에는 100개의 값만 반복해서 등장한다.
